...
固定テンプレートで埋めるよりも、後述の「指針」を満たすことを優先する。差分の性質（新機能追加・バグ修正・リファクタ・依存更新・パフォーマンス改善・ドキュメント改修・テスト追加 等）に応じて、見出し構成・装飾・粒度は LLM の判断で柔軟に組み立ててよい。
...
たとえば「認証ミドルウェア追加」なら **外向きの挙動**（未認証リクエストがどう扱われるようになるか）か **コードの内部品質軸**（既存ミドルウェア群との責務分離）のどちらが支配的かを選ぶ、といった当てはめを行う。「DB クエリの N+1 解消」なら **外向きの挙動**（レスポンス時間がどう変わるか）か **コードの内部品質軸**（クエリ発行箇所の集約方針）のどちらか、といった具合に PR ごとに当てはめを行う。
...
2. **短く・スキャンしやすく書く**
   ...
   - 1 項目 1 論点を保つ。論点が増えるならサブリストで段落化する
...
5. **設計判断は理由とセットで残す**
   - 「なぜこの場所・この設計・この仕様にしたか」を書く。レビュワーの「なぜ別案ではないのか？」を先回りで潰す
