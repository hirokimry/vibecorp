...
概要欄の語り口は「この PR で何が・誰のために変わるか」で大きく変わる。本文作成に入る前に、**変化の主役を 1〜2 行で言語化**しておく。

主役は典型的には次のいずれかになるが、列挙したものに縛られなくてよい:

- **人**（CEO・C*O・SM・運営担当者・利用者 等の体験や操作）
- **外向きの挙動**（スキルの出力・hook の挙動・CLI のメッセージ・PR 本文・通知・画面表示）
- **コードの内部品質軸**（責務分離・依存方向・テスタビリティ・複雑度・拡張点）
- **再現していた事象**（バグ修正の場合）
- **その他**（ドキュメント読者・運用フロー・デプロイ体験・セキュリティ姿勢 等、PR の性質に応じて自由に設定）

vibecorp の差分は Bash スクリプト・hook・skill・rules 改修が中心になりやすいため、たとえば「hook 追加」なら **外向きの挙動**（何が止められるようになるか）か **コードの内部品質軸**（既存 hook 群との責務分離）のどちらが支配的かを選ぶ、といった当てはめを行う。
...
固定テンプレートで埋めるよりも、後述の「指針」を満たすことを優先する。差分の性質（hook 追加・スキル新設・rules 改修・ドキュメント等）に応じて、見出し構成・装飾・粒度は LLM の判断で柔軟に組み立ててよい。
...
3. **変化の主役を主語にする**
   - ステップ4で言語化した主役を、概要欄の中心的な主語に据える
   - 主役が **人や外向きの挙動** なら、クラス名・関数名・ファイルパスなどの実装語彙は原則使わない
   - 主役が **コードの内部品質軸** なら、形容詞での曖昧表現を避け「**何が・どこから・どこへ・どう動いたか**」を構造として示す
   - 主役が **再現していた事象** なら、「どんな条件で何が起きていたか」「修正後は何が起きなくなるか」を書く
   - 「○○関数を追加した」だけ／「より見通しが良くなった」だけで終わらせない
...
どの見出しを使うかは、ステップ4で言語化した「変化の主役」を示せるかで判断する。種別と見出しを機械的に対応させない。

#### 禁止パターン

- ❌ **変化の主役と無関係な実装詳細の羅列** で終わる
  - 人や外向きの挙動が主役なのに「関数追加・型分割・カラム追加」だけを並べる
