...
> [!IMPORTANT]
> このスキルは GitHub PR を **新規作成 / 更新する単一責務のスキル** として動作する。
> PR タイトル・本文は CEO が読むため `.claude/rules/communication.md` の **動作主語** ルール（「〜になった／〜できるようになった」）に従う。
> PR には **intent ラベルを付与しない**（Issue #575: intent の SoT は Issue ラベル）。
> Issue リンクは `Closes #N` / `Refs #N`（`#N` 形式必須、URL 形式は `close-on-feature-merge.yml` の抽出対象外）。
...
概要欄の語り口は「この PR で何が・誰のために変わるか」で大きく変わる。本文作成に入る前に、**変化の主役を 1〜2 行で言語化**しておく。これ以降の「指針」「使える見出し要素」「禁止パターン」は、ここで定めた主役を軸に運用する。
...
4. **PR の位置づけを明示する**
   ...
   - **スコープ外**（このPRでやらないこと・別 Issue に切り出した内容）を明示し、レビュワーの期待値をそろえる。
...
- **PR タイトル**: `gh issue view {番号} --json title` で取得した Issue タイトルを使用する。
- **プッシュと PR 作成の連結**: `&&` で連結実行する。
- **チェックボックス**: 自動でチェックしない。
- **draft モード不使用**: レビュー可能な状態で作成する。
- **一時ファイル禁止**: 標準入力から body 設定する。
- `--force` は使用しない。
- 明示的な要求なしに既存 PR を閉じない。
...
- ワークフロー規約（PR 本文の Issue リンク）: `.claude/rules/workflow.md`
...
- 関連スキル: `/vibecorp:ship` / `/vibecorp:pr-fix` / `/vibecorp:pr-fix-loop`
