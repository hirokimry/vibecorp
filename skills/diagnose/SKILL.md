...
**フォールバック動作**: `claude-code-guide` 利用不可時のスキップ動作を以下のとおり定める。

- `claude-code-guide` が利用不可（外部依存の障害等）の場合は 4d をスキップする
- スキップ時に 4a / 4b / 4c は継続する
- スキップした事実はステップ 8 の候補一覧レポートに「4d: スキップ（claude-code-guide 利用不可）」として明記する
