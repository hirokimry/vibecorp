...
ステップ 1 のファイル一覧を、`document-writing.md` の **指針 7 項目** と **禁止 7 項目** に対して 1 件ずつ突き合わせる。

### 照合観点（指針 MUST 7 項目）

| # | 観点 | 検出方法 |
|---|------|---------|
| 1 | 読者像を冒頭で固定する | 冒頭 5 行に読者像の言及があるか |
| 2 | スキャンで要点が掴める構造 | 冒頭コールアウトと見出し絵文字の有無 |
| 3 | 1 文 50 文字以下 | 句点で区切った 1 文の文字数 |
| 4 | 文末で改行・箇条書きネスト | 1 段落内の連続文の有無 |
| 5 | マークダウン・絵文字活用 | テーブル・コールアウト・状態絵文字の使用 |
| 6 | 設計判断は理由とセット | 「なぜ」の記述の有無 |
| 7 | 汎用語彙で書く | 実装詳細語彙の地の文混入の有無 |

### 照合観点（禁止 7 項目）

| # | 禁止 | 検出方法 |
|---|------|---------|
| 1 | 実装詳細の羅列で終わる | 関数名・ファイルパスの平文連続 |
| 2 | 形容詞止まり | 「より良い」「クリーンな」等の単独使用 |
| 3 | 1 段落に複数論点 | 1 段落 2 主張以上の有無 |
| 4 | 装飾だけの絵文字 | 意味のない位置の絵文字 |
| 5 | 暗黙の前提を残す | 未定義語彙の地の文使用 |
| 6 | 1 文 50 文字超の長文 | 句点で区切った 1 文の文字数 |
| 7 | 複数文を改行せず連結 | 1 段落内の連続平文 |

各ファイルに対し「該当項目」と「該当行番号」を表で記録する。
...
- 50 文字超の長文を 50 文字以下に分割する
- 形容詞止まり・実装詳細羅列を動作主語へ書き換える
...
- **コマンドをそのまま実行する**: `2>/dev/null` 等のフォールバックを付けない。
