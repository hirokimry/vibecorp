...
| **standard** | 上記 + /vibecorp:review-to-rules, /vibecorp:sync-check, /vibecorp:sync-edit, /vibecorp:session-harvest, /vibecorp:harvest-all, /vibecorp:context7 | 上記 + review-to-rules-gate, sync-gate, session-harvest-gate, review-gate | CTO, CPO | Claude Max 定額内 | チーム開発 |
...
| `/vibecorp:pr-fix-loop` | `/vibecorp:pr-fix` を5分間隔で定期実行し、マージまで自動で指摘対応を繰り返す |
...
|---|---|---|---|---|
| `review-to-rules-gate.sh` | standard 以上 | `Bash`（`gh pr merge`） | PR マージ | `/vibecorp:review-to-rules` を実行してスタンプを取得 |
| `sync-gate.sh` | standard 以上 | `Bash`（`git push`） | push | `/vibecorp:sync-check` を実行してスタンプを取得 |
| `session-harvest-gate.sh` | standard 以上 | `Bash`（`gh pr merge`） | PR マージ | `/vibecorp:session-harvest` を実行してスタンプを取得 |
...
| `review-to-rules-gate.sh` | `gh pr merge` | `/vibecorp:review-to-rules` | `~/.cache/vibecorp/state/<repo-id>/review-to-rules-ok` |
| `session-harvest-gate.sh` | `gh pr merge` | `/vibecorp:session-harvest` | `~/.cache/vibecorp/state/<repo-id>/session-harvest-ok` |
