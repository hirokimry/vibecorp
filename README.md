...
COO がその意図を解釈し、領分を判定して割り振る。
**技術は CTO、コストは CFO、安全は CISO、法務は CLO**。

その動線が `/vibecorp:ship <Issue URL>` の 1 行。
Issue を渡すだけで実装 → PR → auto-merge まで一気通貫で進む。
...
診断 → 起票 → 実装 → 知見蓄積のループが実際に回る。
**200 を超えるコミット・9 本の完了エピック** を自走で積み上げた。
