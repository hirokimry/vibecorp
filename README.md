...
`.claude/knowledge/{role}/decisions/` および `{role}/audit-log/` への作業ブランチ直書きは、Edit/Write 層 + Bash 層の 2 つの PreToolUse hook で deny される。書込みは `knowledge/buffer` worktree 経由のみ許可。詳細は [`docs/SECURITY.md`](docs/SECURITY.md#knowledge-ガードレール多層防御) 参照。
...
- CodeRabbit と並走運用可能（2 つのレビュアーが独立して動作。`coderabbit.enabled` と `claude_action.enabled` を両方 `true` で同時実行）
- 認証: `CLAUDE_CODE_OAUTH_TOKEN` シークレットが必要（詳細は `docs/ai-review-auth.md`）
- 既存ファイルがある場合は `--update` で 3-way マージ（利用者カスタマイズは保持）
- `intent-label-check` ジョブが `intent/*` ラベルを 0 個または 2 個以上付与された PR を fail コメント付きで止める（1 PR 1 intent ルールの機械的強制、詳細は `.claude/rules/intent-labels.md` と `docs/conventional-commits.md` を参照）

### AI レビュー Golden Test ワークフロー（`.github/workflows/ai-review-golden-test.yml`）

- `claude_action.enabled: true`（デフォルト）時のみ配置される。`false` 設定時は配置されない（ai-review.yml と連動）
- claude-code-action のレビュー品質リグレッション検知用（既知 PR の期待結果と実出力を比較）
- `paths` トリガー: `REVIEW.md` / `templates/REVIEW.md.tpl` / `.claude/rules/severity/**` / `.claude/rules/review-handling.md` / `.claude/rules/review-observations.md` / `tests/golden/**` の変更時のみ走る（トークン消費とレイテンシー最小化）
- `permissions: contents: read / pull-requests: read` のみ（コメント投稿しないため `write` 不要、`id-token: write` も不要）
- Fork PR / draft PR 除外条件あり
- 認証: `CLAUDE_CODE_OAUTH_TOKEN` シークレットが必要（golden-test スクリプト経由で claude-code-action を起動）
- **0.33.6 互換 (Issue #532)**: 旧版 `copy_workflows()` 経由で配置された `ai-review*.yml` は `vibecorp.lock` に未登録だが、本版からはテンプレートと SHA256 完全一致なら管理下とみなして `enabled: false` で削除する遡及ロジックが入っている（ユーザー編集済みのファイルは残置）
...
`install.sh` が `gh label create` で以下のラベルを自動作成する（既存名は再作成しない）。詳細は `docs/conventional-commits.md` と `.claude/rules/intent-labels.md`:
