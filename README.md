...
🤖 **機能レベルで指示するだけ。専門家チームが遂行し、組織の「判例」が自律的に育つ。**

> [!IMPORTANT]
> このドキュメントは **外部 OSS 読者・導入開発者** 向けの入口ガイドです。
> vibecorp は Claude Code を「**意思決定の歴史を引き継ぐ AI 開発組織**」に変えるプラグインです。
> どのリポジトリにも 2 コマンドで導入でき、詳細仕様は [`docs/specification.md`](docs/specification.md) を参照してください。

---

## 🌟 このプロダクトが本当に凄いこと

AI エージェントを「組織化」したこと自体は手段にすぎない。
凄いのは、**組織化したことで生まれた 3 つの効果** が連動して回り続けることにある。

### ① 経営者の言葉だけで、専門家チームが動く

コードもファイルパスも知らなくていい。「〜できるようにしたい」と **機能レベルで指示するだけ**。
COO がその意図を解釈し、**技術は CTO、コストは CFO、安全は CISO、法務は CLO** へと領分を判定して割り振る。

その動線が `/vibecorp:ship <Issue URL>` の 1 行。Issue を渡すだけで実装 → PR → auto-merge まで一気通貫で進む。

### ② 専門家の判断が「判例」として残り、次の判断に効く（複利）

ここが標準的な AI ツールと決定的に違う。
各役職は判断のたびに **結論・根拠・却下した代替案・過去判断との一貫性** を記録し、
...
| 💰 CFO | `max_issues_per_run` 最適値を「3 へ」→ 再判断で「7 へ」自己撤回（月 $840 試算付き） |
| ⚖️ CLO | 「CodeRabbit 完全網羅」表現に待った（著作権法 2 条・不正競争防止法 2 条 1 項 3 号） |
| 📋 CPO | Pro プラン新設を No-Go（4 段目は「導入の手軽さ」バリューに反する） |
| 🏃 SM | 7 件の同時並列は不可、最大 3 件並列が安全な上限（install.sh 競合を検出） |

> 🤖 **Claude 標準メモリーとの違い**
> 標準メモリーは「一人のアシスタントの記憶」（個人・フラット・上書き）。
> vibecorp は「**複数の専門部署が判例を引き継ぐ組織記憶**」（役職別・時系列・査読を経て蓄積）。
> 判断は `.claude/knowledge/{役職}/decisions/` に四半期ごとに残り、各役職の起動時に読み戻される。

### ③ この組織が、自分自身を自分の仕組みで育てている

vibecorp は自分のコードベースを vibecorp で改善している。
診断 → 起票 → 実装 → 知見蓄積のループが実際に回り、**240 コミット・9 本の完了エピック** を自走で積み上げた。
上の判例集は、その自己改善の過程で AI 自身が書き残したものである。
...
| できること | 動線 |
|------------|----------|
| 🚀 Issue を実装・PR 化・auto-merge まで全自動 | `/vibecorp:ship <Issue URL>` 1 行（上記 ①） |
| 🎁 個人 → チーム → AI 企業まで段階導入 | プリセット 3 段階（minimal / standard / full）。`install.sh --preset full` で AI 企業組織モード |
| 🔒 品質ゲートが常時稼働 | `vibecorp.yml` でも無効化できない必須 hook（上記 🛡️ 土台） |
