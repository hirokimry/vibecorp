...
RULE_INTENT_TPL="${SCRIPT_DIR}/templates/claude/rules/intent-labels.md"
...
assert_file_exists "intent-labels.md（テンプレート版）" "$RULE_INTENT_TPL"
...
for intent in intent/feature intent/bugfix intent/performance intent/security intent/refactor intent/infra intent/docs; do
  ...
  if grep -q -F -- "$intent" "$RULE_INTENT_TPL"; then
    pass "intent-labels.md（テンプレート版）に '$intent' 含む"
  else
    fail "intent-labels.md（テンプレート版）に '$intent' 含まない"
  fi
done
...
assert_file_contains_fixed "テンプレート版も同様" "$RULE_INTENT_TPL" "PR には intent ラベルを付与しない"
