...
# Z4. standard lock: agents が lock に記録される
...
# AI2. --update 時に vibecorp プレースホルダーが正常に置換されること（agents で検証）
create_test_repo
bash "$INSTALL_SH" --name test-proj --preset full 2>/dev/null
R="$TMPDIR_ROOT"

# vibecorp プレースホルダーを含むファイルを agents に配置して --update で再実行
echo '# {{PROJECT_NAME}} テスト' > "$R/.claude/agents/test-placeholder.md"

bash "$INSTALL_SH" --update 2>/dev/null
REMAINING=$(grep -l '{{PROJECT_NAME}}' "$R/.claude/agents/test-placeholder.md" 2>/dev/null || true)
if [ -z "$REMAINING" ]; then
...
# クリーンアップ
rm -f "$R/.claude/agents/test-placeholder.md"
cleanup

# AI2b. 未知のテンプレート構文（docker inspect 等）は誤検知しない（agents で検証）
create_test_repo
bash "$INSTALL_SH" --name test-proj --preset full 2>/dev/null
R="$TMPDIR_ROOT"
...
# {{.HostConfig.Memory}} / {{.State.ExitCode}}' > "$R/.claude/agents/test-docker-template.md"
...
# ファイル内容が書き換えられていないこと
if grep -q '{{.State.Status}}' "$R/.claude/agents/test-docker-template.md"; then
  pass "AI2b: 未知のテンプレート構文を保持する"
else
  fail "AI2b: 未知のテンプレート構文が書き換えられた"
fi
# クリーンアップ
rm -f "$R/.claude/agents/test-docker-template.md"
cleanup
...
TMP_FILES=$(find "$R/.claude/agents/" "$R/skills/" -name '*.tmp' 2>/dev/null || true)
if [ -z "$TMP_FILES" ]; then
