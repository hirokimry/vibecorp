...
# E1-E3. hooks は plugin native 配布 (#716) に移行済のため install.sh は配置しない
# 検証は plugin 経由（hooks.json）で実施される前提のため、ここでは検証対象外
...
# E7. vibecorp.lock にマニフェスト (#735 で v3 形式: agents セクションも廃止)
assert_file_contains "vibecorp.lock が v3 形式 (format_version: 3)" "$R/.claude/vibecorp.lock" "format_version: 3"
assert_file_contains "vibecorp.lock に rules マニフェスト" "$R/.claude/vibecorp.lock" "code-comments.md"

# E8-E9. settings.json には hooks ブロックが書かれない（plugin native 配布 #716）
assert_file_not_contains "settings.json に PreToolUse なし（plugin 配布化）" "$R/.claude/settings.json" "PreToolUse"
...
# F1-F3, F5. hooks は plugin native 配布に移行済のため、preset 別 hook ファイル削除 / settings.json
# からの hooks エントリ判定は不要。skills/ の preset 別削除のみ確認する
...
# ============================================
echo ""
echo "=== H. settings.json マージ（hooks は plugin native 配布化で対象外） ==="
# ============================================

# H1. 既存 settings.json にユーザー独自 hooks があっても、install.sh は触らずに残す
create_test_repo
...
# H2. ユーザー独自フックは plugin native 配布化後も残る
assert_file_contains "ユーザー独自フックが残る" "$R/.claude/settings.json" 'my-custom-hook.sh'

# H3. install.sh は permissions / marketplaces / enabledPlugins を併合する
assert_file_contains "marketplace が併合される" "$R/.claude/settings.json" 'extraKnownMarketplaces'
assert_file_contains "enabledPlugins が併合される" "$R/.claude/settings.json" 'enabledPlugins'
...
# ============================================
echo ""
echo "=== K. 既存フック保持（独自フックは plugin native 配布化後も残る） ==="
# ============================================

# K1. ユーザー独自フックが install 後も残る（install.sh は .claude/hooks/ を touch しない）
create_test_repo
...
# K2. vibecorp 由来 hook は plugin 経由で配布されるため .claude/hooks/ には書かれない
assert_file_not_exists "vibecorp 由来 hook は .claude/hooks/ に配置されない" "$R/.claude/hooks/protect-files.sh"
...
# W1. agents は plugin native 配布 (#737 / #735) に移行済のため、minimal/standard/full いずれでも
# install.sh は .claude/agents/ を作成しない（呼出ゲートは vibecorp.yml plan.review_agents で行う）
create_test_repo
bash "$INSTALL_SH" --name test-proj 2>/dev/null
R="$TMPDIR_ROOT"

if [ ! -d "$R/.claude/agents" ]; then
  pass "minimal プリセットで .claude/agents/ は作成されない（plugin native 配布）"
else
  fail "minimal プリセットで .claude/agents/ が作成されている"
fi

# W2. lock に agents セクション自体が無い（lock 形式 v3、#735）
assert_file_not_contains "lock に agents セクションなし（v3）" "$R/.claude/vibecorp.lock" "agents:"
assert_file_not_contains "minimal の lock に cto.md なし" "$R/.claude/vibecorp.lock" "cto.md"

cleanup

# W4. plugin ルートのテンプレート（A1 で `agents/` へ移動済）にエージェントファイルが存在する
...
# Z1. standard 新規インストール: agents は plugin native 配布 (#737 / #735) のため作成されない
create_test_repo
bash "$INSTALL_SH" --name test-proj --preset standard 2>/dev/null
R="$TMPDIR_ROOT"

if [ ! -d "$R/.claude/agents" ]; then
  pass "standard: .claude/agents/ は作成されない（plugin native 配布）"
else
  fail "standard: .claude/agents/ が作成されている"
fi

# Z2-Z5. hooks は plugin native 配布 (#716) に移行済のため、preset 別 hook ファイル配置 / lock 記録 /
# settings.json への hook エントリは検証対象外
...
# Z4. standard lock: agents セクション自体が無い（lock 形式 v3、#735）
assert_file_not_contains "standard lock: agents セクションなし（v3）" "$R/.claude/vibecorp.lock" "agents:"
assert_file_not_contains "standard lock: cto.md なし" "$R/.claude/vibecorp.lock" "cto.md"
...
# Z7. minimal → standard アップグレード: agents は plugin native 配布のため .claude/agents/ は作成されない
create_test_repo
bash "$INSTALL_SH" --name test-proj --preset minimal 2>/dev/null
R="$TMPDIR_ROOT"

# minimal 状態を確認（agents 不在）
assert_file_not_exists "アップグレード前: agents なし" "$R/.claude/agents/cto.md"

# standard にアップグレード
bash "$INSTALL_SH" --update --preset standard 2>/dev/null

# agents は plugin native 配布のため作成されない（呼出ゲートは vibecorp.yml plan.review_agents で行う）
assert_file_not_exists "アップグレード後: .claude/agents/cto.md は作成されない（plugin native）" "$R/.claude/agents/cto.md"
...
# Z10. minimal → standard アップグレード: lock に agents セクションなし（v3）
assert_file_not_contains "アップグレード後: lock に agents セクションなし（v3）" "$R/.claude/vibecorp.lock" "agents:"

cleanup

# Z12. standard → standard 更新: 正常に更新できる（agents は plugin native のため作成されない）
create_test_repo
bash "$INSTALL_SH" --name test-proj --preset standard 2>/dev/null
R="$TMPDIR_ROOT"
...
# agents は plugin native 配布のため作成されない
assert_file_not_exists "standard 更新後: .claude/agents/cto.md は作成されない（plugin native）" "$R/.claude/agents/cto.md"
...
# T1. hooks は plugin native 配布 (#716) に移行済のため、install.sh による .claude/hooks/ 配置 /
# yml トグルでの hook ファイル削除は対象外。yml トグルは各 hook が自己 skip する形で機能する
...
# T3-T6. hooks は plugin native 配布 (#716) に移行済のため、settings.json の hooks エントリ判定 /
# install.sh による .claude/hooks/ 配置と削除は検証対象外

# T6_skill. 同名ユーザー skill が --update で削除されない（既存挙動の確認）
create_test_repo
R="$TMPDIR_ROOT"
...
# PLAN4. full プリセット: vibecorp.yml の plan.review_agents に cost / legal が含まれる
# （agents は plugin native 配布 #737 / #735 のため .claude/agents/ には配置されない。
# 呼出ゲートは vibecorp.yml plan.review_agents が担う）
create_test_repo
bash "$INSTALL_SH" --name test-proj --preset full > /dev/null 2>&1
R="$TMPDIR_ROOT"
assert_file_contains "PLAN4a: full vibecorp.yml の plan.review_agents に cost が含まれる" "$R/.claude/vibecorp.yml" "- cost"
assert_file_contains "PLAN4b: full vibecorp.yml の plan.review_agents に legal が含まれる" "$R/.claude/vibecorp.yml" "- legal"
cleanup

# PLAN5. standard プリセット: vibecorp.yml の plan.review_agents に cost / legal が含まれない
# （architect / security / testing のみ）
create_test_repo
bash "$INSTALL_SH" --name test-proj --preset standard > /dev/null 2>&1
R="$TMPDIR_ROOT"
assert_file_not_contains "PLAN5a: standard vibecorp.yml の plan.review_agents に cost なし" "$R/.claude/vibecorp.yml" "- cost"
assert_file_not_contains "PLAN5b: standard vibecorp.yml の plan.review_agents に legal なし" "$R/.claude/vibecorp.yml" "- legal"
assert_file_contains "PLAN5c: standard vibecorp.yml の plan.review_agents に architect が含まれる" "$R/.claude/vibecorp.yml" "- architect"
cleanup

# PLAN6. minimal プリセット: .claude/agents/ が作成されない（plugin native 配布）
create_test_repo
bash "$INSTALL_SH" --name test-proj --preset minimal > /dev/null 2>&1
R="$TMPDIR_ROOT"
if [ ! -d "$R/.claude/agents" ]; then
  pass "PLAN6: minimal プリセットで .claude/agents/ は作成されない（plugin native 配布）"
else
  fail "PLAN6: minimal プリセットで .claude/agents/ が作成されている"
fi
cleanup
...
# AI1-AI3. agents / skills が共に plugin native 配布 (#716 / #737 / #735) に移行済のため、
# install.sh の copy_managed_files() 内 target_dirs ループ (L904) は実質的に dead code となった。
# 代わりに docs / knowledge / vibecorp.yml は別経路で個別にプレースホルダー置換される
# （L2269 / L2304 / L2486 / L2527 / L1164 で実装）。docs プレースホルダー置換は X5 セクションで
# `assert_file_not_contains "specification.md にプレースホルダーなし" ... '{{.*}}'` 等が既に検証している。

create_test_repo
bash "$INSTALL_SH" --name test-proj --preset full --language ja 2>/dev/null
R="$TMPDIR_ROOT"

# AI1. docs/ 配下にプレースホルダーが残っていないこと（X5 と重複するが明示的に検証）
REMAINING=$(grep -rl '{{PROJECT_NAME}}\|{{PRESET}}\|{{LANGUAGE}}' "$R/docs/" 2>/dev/null || true)
if [ -z "$REMAINING" ]; then
  pass "AI1: docs/ 内にプレースホルダーが残っていない"
else
  fail "AI1: docs/ 内にプレースホルダーが残っている: $REMAINING"
fi

# AI3. install 後に .tmp ファイルが残っていないこと（agents/skills が無くても docs/knowledge の置換は走る）
TMP_FILES=$(find "$R/.claude/" "$R/docs/" -name '*.tmp' 2>/dev/null || true)
if [ -z "$TMP_FILES" ]; then
...
# stdin を /dev/null に明示的に redirect することで、テスト実行環境の stdin が tty であっても
# install.sh の [[ -t 0 ]] が必ず false になり、y/N プロンプトでハングしないようにする。
create_test_repo
STDERR_OUT=$(ANTHROPIC_API_KEY=test-dummy-key bash "$INSTALL_SH" --name test-proj --preset full </dev/null 2>&1 1>/dev/null)
...
create_test_repo
STDERR_OUT=$(env -u ANTHROPIC_API_KEY bash "$INSTALL_SH" --name test-proj --preset full </dev/null 2>&1 1>/dev/null)
if echo "$STDERR_OUT" | grep -q -e "$WARN_A_MARKER"; then
...
create_test_repo
STDERR_OUT=$(ANTHROPIC_API_KEY=test-dummy-key bash "$INSTALL_SH" --name test-proj --preset minimal </dev/null 2>&1 1>/dev/null)
if echo "$STDERR_OUT" | grep -q -e "$WARN_A_MARKER"; then
...
create_test_repo
STDERR_OUT=$(ANTHROPIC_API_KEY=test-dummy-key bash "$INSTALL_SH" --name test-proj --preset standard </dev/null 2>&1 1>/dev/null)
if echo "$STDERR_OUT" | grep -q -e "$WARN_A_MARKER"; then
...
if require_darwin "WARN5: full + Darwin + sandbox 未有効 → 警告 B が表示される"; then
  ...
  STDERR_OUT=$(env -u VIBECORP_ISOLATION HOME="$FAKE_HOME" XDG_CACHE_HOME="$FAKE_HOME/.cache" \
    bash "$INSTALL_SH" --name test-proj --preset full </dev/null 2>&1 1>/dev/null)
...
create_test_repo
STDERR_OUT=$(env -u VIBECORP_ISOLATION bash "$INSTALL_SH" --name test-proj --preset minimal </dev/null 2>&1 1>/dev/null)
if echo "$STDERR_OUT" | grep -q -e "$WARN_B_MARKER"; then
...
create_test_repo
EXIT_CODE=0
ANTHROPIC_API_KEY=test-dummy-key bash "$INSTALL_SH" --name test-proj --preset full </dev/null 2>/dev/null || EXIT_CODE=$?
