#!/bin/bash
# test_notifications_extract_all_skill.sh — /vibecorp:notifications-extract-all スキル新設（Issue #645）の静的検証
# 使い方: bash tests/test_notifications_extract_all_skill.sh
...
SKILL_FILE="${SCRIPT_DIR}/skills/notifications-extract-all/SKILL.md"
DIST_SKILL_FILE="${SCRIPT_DIR}/.claude/vibecorp-base/skills/notifications-extract-all/SKILL.md"
EXTRACTION_RULE="${SCRIPT_DIR}/.claude/rules/notification-prompt-extraction.md"

# ============================================
echo "=== skills/notifications-extract-all/SKILL.md が存在する ==="
# ============================================

assert_file_exists "skills/notifications-extract-all/SKILL.md が存在する" "$SKILL_FILE"
...
assert_file_contains "name キーが notifications-extract-all" "$SKILL_FILE" "^name: notifications-extract-all$"
...
# description 行から「...」で囲まれたトリガー語句を抽出
...
assert_file_contains "トリガー語句: /notifications-extract-all" "$SKILL_FILE" "/notifications-extract-all"
...
# ============================================
echo "=== 検出対象（.github/workflows/** / hooks/**）が記述されている ==="
# ============================================

assert_file_contains "検出対象: .github/workflows/**/*.yml" "$SKILL_FILE" "\.github/workflows/\*\*/\*\.yml"
assert_file_contains "検出対象: .github/workflows/**/*.yaml" "$SKILL_FILE" "\.github/workflows/\*\*/\*\.yaml"
assert_file_contains "検出対象: hooks/**/*.sh" "$SKILL_FILE" "hooks/\*\*/\*\.sh"
assert_file_contains "検出パターン: --body" "$SKILL_FILE" "\-\-body"
assert_file_contains "検出パターン: heredoc 言及" "$SKILL_FILE" "heredoc"

# ============================================
echo "=== 除外ルールが明示されている ==="
# ============================================

assert_file_contains "除外: permissions: セクション" "$SKILL_FILE" "permissions:"
assert_file_contains "除外: secrets: セクション" "$SKILL_FILE" "secrets:"
assert_file_contains "除外: ANSI-C クォート" "$SKILL_FILE" "ANSI-C"
assert_file_contains "除外: \$(cat ...) 動的展開" "$SKILL_FILE" '\$(cat'
assert_file_contains "除外: 短い通知文（閾値以下）" "$SKILL_FILE" "閾値"
...
assert_file_contains "不可領域 6（CI エージェント）への言及" "$SKILL_FILE" "不可領域 6"
...
# ============================================
echo "=== 委譲先 CISO が明示されている ==="
# ============================================

assert_file_contains "委譲先: CISO" "$SKILL_FILE" "CISO"
assert_file_contains "CISO 委譲の理由（CEO 向け文面 / 情報露出）" "$SKILL_FILE" "情報露出"

# ============================================
echo "=== 関連ルールへの参照（notification-prompt-extraction.md / communication.md / markdown.md） ==="
# ============================================
...
assert_file_contains "兄弟スキル /prompts-extract-all への参照" "$SKILL_FILE" "/prompts-extract-all"
...
if [[ -f "$DIST_SKILL_FILE" ]]; then
  ...
  echo "  SKIP: .claude/vibecorp-base/skills/notifications-extract-all/SKILL.md が未生成（self-install されていないため検証スキップ）"
fi
