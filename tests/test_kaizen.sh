#!/bin/bash
# test_kaizen.sh — /kaizen スキルの統合テスト
# 使い方: bash tests/test_kaizen.sh
# CI: GitHub Actions で自動実行

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
SKILL_MD="${SCRIPT_DIR}/templates/claude/skills/kaizen/SKILL.md"
PASSED=0
FAILED=0
TOTAL=0
...
# 2. name が定義されている
assert_file_contains "frontmatter に name: kaizen がある" "$SKILL_MD" "^name: kaizen"
...
# 6. kaizen-active スタンプ作成がある
assert_file_contains "kaizen-active スタンプ作成がある" "$SKILL_MD" "kaizen-active"
...
# 14. スタンプ削除の記述がある
assert_file_contains "スタンプ削除の記述がある" "$SKILL_MD" "kaizen-active.*削除"
...
# 18. kaizen ラベルの付与がある
assert_file_contains "kaizen ラベルの付与がある" "$SKILL_MD" "kaizen"
