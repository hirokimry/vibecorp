...
# B1. minimal プリセットでは context7 が削除される（plugin skills）
create_test_repo
bash "$INSTALL_SH" --name test-proj --preset minimal 2>/dev/null
assert_dir_not_exists "minimal: context7 スキルが削除される" \
  "$TMPDIR_ROOT/skills/context7"
cleanup

# B2. standard プリセットでは context7 が配置される（plugin skills）
create_test_repo
...
# B3. full プリセットでは context7 が配置される（plugin skills）
create_test_repo
...
# D1. vibecorp.yml で context7 を無効化すると配置されない（plugin skills）
create_test_repo
bash "$INSTALL_SH" --name test-proj --preset standard 2>/dev/null
