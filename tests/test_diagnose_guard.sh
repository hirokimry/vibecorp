...
# Issue #703: hook が ${HOOK_DIR}/../lib/ で lib を解決するようになり、templates/claude/hooks/ から実行する経路では templates/claude/lib/ に lib が必要になる（plugin native 配布後の runtime 配置と同じ構造を再現する）
# shellcheck disable=SC1091
