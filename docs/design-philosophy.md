...
これにより利用者は `vibecorp.yml` の `skip_paths` を編集するだけで両方の AI レビュー経路の除外設定が同期し、CodeRabbit と claude-code-action の skip 設定が乖離しない。`REVIEW.md` 自体は claude-code-action のプロンプト用で、判定基準・観点の実体は `.claude/rules/severity/*` と `.claude/rules/review-handling.md` / `review-observations.md` を Source of Truth として参照させる（`REVIEW.md` は実体ルールを持たない）。
...
- 既存ファイルは `merge_or_overwrite` の 3-way マージで利用者カスタマイズを保持、`enabled: false` 時は管理下ファイルと base snapshot を削除して AI レビューを実質無効化
