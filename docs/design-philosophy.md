...
`VIBECORP_SANDBOXED=1` 環境変数と PPID チェーン検証の AND 条件で passthrough を判断する。環境変数単独では外部注入によるバイパスが可能なため、祖先プロセスに `sandbox-exec` (Darwin) / `bwrap` (Linux) が存在することをあわせて確認する。

### fail-closed 設計

sandbox 起動が失敗した場合、passthrough（隔離なしで起動）は **絶対にしない**。`bwrap` の user namespace 無効・kernel ENOSYS・AppArmor 拒否、`sandbox-exec` のプロファイル不正等で起動失敗した場合、プロセスは非ゼロで終了する。`claude` shim の `is_inside_sandbox` が祖先 sandbox プロセスの検出に失敗（偽陰性）した場合も、`VIBECORP_SANDBOXED=1` との AND 条件が成立せず通常 ISOLATION 経路に進む。

### OS ディスパッチャの抽象化

OS 判定を `vibecorp-sandbox` に閉じ込める。両 OS で実隔離が稼働する:
...
Linux 隔離レイヤの実装基盤として `bwrap-args.sh` を独立スクリプトに分離している。これにより:
