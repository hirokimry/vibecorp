...
これにより利用者は `vibecorp.yml` の `skip_paths` を編集するだけで両方の AI レビュー経路の除外設定が同期し、CodeRabbit と claude-code-action の skip 設定が乖離しない。`REVIEW.md` 自体は claude-code-action のプロンプト用で、判定基準・観点の実体は `.claude/rules/review-criteria.md` を Source of Truth として参照させる（`REVIEW.md` は実体ルールを持たない）。
