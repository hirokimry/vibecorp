...
vibecorp は組織規模に応じた 3 段階プリセット（minimal / standard / full）を提供する。本セクションは設計思想観点の要約に絞り、配置されるスキル / フック / エージェントの具体一覧と auto 体験射程・SKIP 性は [`docs/specification.md`](specification.md) と [`README.md`](../README.md) を Source of Truth とする。
...
> 通常の PR レビューを支える `.coderabbit.yaml` は引き続き配布する（[CodeRabbit 設定（`.coderabbit.yaml`）](#coderabbit-設定-coderabbityaml) を参照）。本セクションが対象とするのは「統合問題を AI レビューで拾う目的の追加配布」であり、それは行わない。
