...
- **full**: 上記 + /diagnose, /autopilot, /spike-loop, /ship-parallel

## full プリセットのコンテナ隔離

full プリセットの自律実行スキル（spike-loop, ship-parallel, autopilot）は Docker コンテナ内で実行する。これは設計上の必須要件であり、オプションではない。

### 設計判断

- **Docker が full の必須依存**: `install.sh` が `check_docker()` で Docker CLI + デーモンの存在を検証し、未導入なら案内メッセージを出力して exit 1 する。minimal / standard には影響しない
- **インストール時にイメージをビルド**: `prepare_docker_image()` が `vibecorp/claude-sandbox:dev` をビルドする。スキル実行時の遅延ビルドは並列実行での競合リスクがあるため却下した
- **コンテナ内では `--permission-mode dontAsk`**: 無人実行のためユーザー確認をスキップする。これが安全に機能するのはコンテナ隔離があるため
- **SECURITY.md の最低条件が必須**: docker.sock 非マウント、capabilities 最小化、read-only rootfs、secrets の bind mount 注入など、全条件を満たすコンテナ設定でなければ NO-GO

### minimal / standard との関係

minimal と standard は Docker なしで完全に動作する。コンテナ隔離は full プリセット専用の機能であり、下位プリセットに持ち込まない。これは「段階的成長」バリューに沿った設計であり、コンテナ化は全エージェントが COO 主導で動く full の前提アーキテクチャとして位置づける。
...
いずれも `$CLAUDE_PROJECT_DIR/.claude/state/` 配下のステートファイル（`{gate名}-ok` 形式）で状態管理。ステートは確認後に自動削除される（ワンタイム）。worktree ごとに `CLAUDE_PROJECT_DIR` が異なるため、ブランチ単位で自然に分離される。
