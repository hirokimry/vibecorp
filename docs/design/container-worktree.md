...
worktree 作業ディレクトリと、リポジトリの `.git/` を別々にマウントし、worktree 内の `.git` ポインタを書き換える。
...
worktree の `.git` ポインタをコンテナ内パスに書き換えるのではなく、コンテナ側で git の `GIT_DIR` / `GIT_COMMON_DIR` 環境変数を設定する方式を採用する。これによりホスト側のファイルを変更せずにコンテナ内で git が正しく動作する。
...
- `git push` は HTTPS 経由で GitHub API を叩くため、ファイルレベルのロック競合なし
...
### 方式: read-only マウント + GH_TOKEN 環境変数

```bash
# gh CLI 設定の read-only マウント
...
したがって、`$HOME/.config/gh` のマウントは**省略可能**。`GH_TOKEN` 環境変数のみで gh CLI は動作する。

**推奨構成**: `.config/gh` マウントを省略し、`/run/secrets/github_token` からの `GH_TOKEN` 展開のみで運用する。

```bash
# 最小構成（.config/gh マウント不要）
--mount type=bind,source="$PWD/secrets/github_token",target=/run/secrets/github_token,readonly
```
