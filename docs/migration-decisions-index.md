---
description: C*O decisions.md を decisions-index.md + 四半期アーカイブ 2 段構成に移行する手順
---

> [!IMPORTANT]
> 本ガイドは **🛠️ vibecorp 導入リポジトリの管理者・運用者** が読者。
> 旧 `decisions.md` 単一ファイルから新 `decisions-index.md` + `decisions/YYYY-QN.md` 構造へ安全に移行できるようになる。
> 既存環境を壊さず、エージェント側 fallback で旧構造も読めるようになる。

# 📖 C\*O `decisions.md` → `decisions-index.md` + 四半期アーカイブ 移行ガイド

`.claude/knowledge/{role}/decisions.md` の肥大化問題を解消する移行手順を定める。
...
| ファイル | 役割 | Read タイミング |
|---|---|---|
| `decisions-index.md` | 1 エントリ 1 行のサマリを新しい順で並べる目次 | 毎起動 |
| `decisions/YYYY-QN.md` | 判断の詳細本文を四半期単位で保持するアーカイブ | 関連エントリがあるときのみ |
...
> [!NOTE]
> 既に `.claude/knowledge/{role}/decisions.md`（旧形式）を持つ vibecorp 導入リポジトリの管理者向けの手順である。

### 👥 対象読者

- 運用中に agents が `decisions.md` に追記した履歴を持つリポジトリ
- `install.sh --update` 実行済みで agents は新版に差し替え済みだが `decisions.md` は残存している状態（install.sh は knowledge 配下を削除しない）
...
step 4（記録）も同様に fallback する。

両方存在する場合は新形式（`decisions-index.md`）を優先する。

### 移行タイミング

肥大化していない legacy 環境は無理に移行しなくてよい。

- 対象例: CFO / SM で数エントリのみ。

- 現状の `decisions.md` を使い続けられる。
