...
> [!IMPORTANT]
> 読者像は worktree モード対応スキルを実装・運用する開発者・利用者。
> 複数スキルが `--worktree <path>` をサポートする。
> 本ドキュメントは worktree モードの統一的な実装パターンと設計背景の **Source of Truth**。

## 1️⃣ 概要

worktree モードは、`git worktree` で作成された独立ディレクトリ内で全操作を実行する仕組み。

主な用途は並列 ship。

- 🎯 `/vibecorp:ship-parallel` が複数 Issue を並列処理する際に使用する。
- 🛡️ 各 Issue の作業空間が独立し、互いの diff が混ざらない。

## 2️⃣ 2 つの実行パターン

worktree 内でのファイル操作は、ツールの種類に応じて 2 つのパターンに分かれる。

### 🅰️ パターン 1: `cd <path> && command`（Bash コマンド用）

シェルコマンドはカレントディレクトリに依存する。

`cd` で worktree に移動してから実行する。
...
- 🔁 `git -C <path>` は git コマンドのみ対応。
  - `gh` 等には使えない。
- 🌐 `cd && command` なら全コマンドに統一的に適用できる。
- ✨ ワンライナーで完結し、状態を引きずらない。

### 🅱️ パターン 2: 絶対パス指定（Read / Write / Edit 用）

Claude Code の Read / Write / Edit ツールは CWD に依存しない。

`<path>/` を基準とした絶対パスを直接指定する。
...
> [!NOTE]
> 計画ファイル（plans/）は worktree 内ではなく `~/.cache/vibecorp/plans/<repo-id>/` に配置する（#334）。
> `<repo-id>` は worktree パスごとに一意の値になる。
> → worktree 間で計画ファイルは自動的に分離される。

#### 🎯 選定理由

- ✅ Read / Write / Edit は絶対パスを受け取るツール。
  - `cd` は不要。
- 🛡️ CWD の状態に依存しない。
  - 並列実行時にも安全。

## 3️⃣ `--worktree <path>` の引き継ぎルール
...
- 📝 各スキルの SKILL.md に「worktree モード」セクションを記載する。
  - パターン 1・2 の適用方法を明記する。

## 4️⃣ 対応スキル一覧

以下のスキルが `--worktree <path>` に対応する。
...
## 5️⃣ ship-parallel の手動 worktree + rsync 方式

### 📚 背景

`/vibecorp:ship-parallel` は複数 Issue を並列に ship する。

各 Agent が独立した作業ディレクトリを持つ必要がある。

### ❌ Agent の `isolation: "worktree"` を使わない理由

Claude Code の Agent ツールには `isolation: "worktree"` オプションがあるが、以下の理由で採用しなかった。

- 🚫 **TeamCreate との非互換**: `isolation: "worktree"` は TeamCreate 下で機能しない（検証済み）。
- 🚫 **skills / hooks の同期が不確実**: Agent が自動作成する worktree には `.claude/` の内容が同期されない可能性がある。
...
|---|:---:|:---:|:---:|
| `isolation: "worktree"` | ❌ 不可 | ⚠️ 不確実 | ❌ 不可 |
...
- 🏷️ プロジェクト名をプレフィックスに付ける。
  - → 他プロジェクトと衝突しない。
- 🔁 ブランチ名はスラッシュ (`/`) をアンダースコア (`_`) に変換してディレクトリ名にする。

### 🤖 Agent 起動

Agent の起動方式を以下に整理する。

- ✅ 各 Agent は `isolation` なしで起動する。
- 🎯 `/vibecorp:ship --worktree <path>` で全操作を worktree 内に限定する。
- 🔁 TeamCreate + SendMessage により双方向通信が可能。

## 6️⃣ worktree ライフサイクル管理
...
worktree 削除の安全基準を以下に整理する。

- ❌ **未コミット変更がある worktree は自動削除しない**。
- 🧪 Agent worktree は「未コミット変更なし + ベースブランチとの差分なし」の場合のみ孤立と判定する。
  - 該当時のみ削除対象とする。
- 🚫 `git branch -D`（強制削除）は使用しない。

## 7️⃣ スキル実装者向けガイドライン

新しいスキルに worktree モードを追加する場合の手順を以下に整理する。
