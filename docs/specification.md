...
| `/vibecorp:review` | Claude Code CLI 直接呼び出し + カスタムレビュアーで変更差分をローカルレビュー（CodeRabbit CLI 依存は撤去済み、Issue #499） |
...
#### 🦅 AI レビュー（vibehawk へ外部化、Issue #531）

vibecorp は AI レビュー本体を **vibehawk へ移譲** した。

- vibecorp は `.vibehawk.yaml` の生成と `npx vibehawk setup` 導線提供に徹する（workflow は vendoring しない）
- レビュー実体（auto-review / approve 切替 / auto-resolve / 日本語）は vibehawk ツール側に内蔵される
- 旧 claude-code-action 実装（`REVIEW.md` / `ai-review.yml` / `ai-review-golden-test.yml`）は配布されなくなった
  - `--update` 時に vibecorp 管理下のこれらファイルは削除される（ユーザー編集済みは残置）
- レビュアーは `vibehawk` / `coderabbit` の独立トグルで選択する（**デフォルト vibehawk-only**: vibehawk=on / coderabbit=off）
- 詳細は [`docs/ai-review-dependency.md`](ai-review-dependency.md) / [`docs/ai-review-auth.md`](ai-review-auth.md) を参照

##### 🦅 vibehawk 設定（`.vibehawk.yaml`）

- `vibehawk.enabled: true`（デフォルト）時のみ `install.sh` が生成。`false` 設定時は管理下ファイルを削除
- vibecorp 独自の判断軸（PR が対応 Issue の全要件を満たすこと）を `reviews.path_instructions` に注入
- `vibehawk.skip_paths` から `reviews.path_filters`（除外パス）を自動生成
- レビュー稼働には別途 `npx vibehawk setup`（GitHub App + 3 secrets + workflow 配布 PR）が必要
  - 3 secrets: `VIBEHAWK_APP_ID` / `VIBEHAWK_PRIVATE_KEY` / `CLAUDE_CODE_OAUTH_TOKEN`
  - required check `vibehawk` の branch protection 登録は CEO の操作（install.sh は登録しない）

##### 🤖 CodeRabbit 設定（`.coderabbit.yaml`）

- `coderabbit.enabled: true` 時のみ配置される（デフォルトは `false`）
...
##### 🏷️ Issue intent ラベルチェック（レビュアー非依存）
...
> ⚠️ **状態 (Issue #531)**: 配布デフォルトは vibehawk-only だが、vibecorp 本体は own-repo の vibehawk 活性化（別 Issue）まで CodeRabbit 単独運用を継続する（意図的な不一致、[`docs/ai-review-dependency.md`](ai-review-dependency.md) を参照）。
...
| ファイル種別 | 更新時の挙動 |
|---|---|
| **hooks** | plugin native 配布（#716）。install.sh は配置・更新せず、Claude Code が plugin cache 経由で自動検出する |
| **skills** | plugin native 配布（#358）。install.sh は配置・更新せず、Claude Code が plugin cache 経由で自動検出する |
| **agents** | plugin native 配布（#737 / #735）。install.sh は配置・更新せず、Claude Code が plugin cache 経由で自動検出する |
...
- `templates/claude/.gitignore` の `# ---- machine-specific artifacts ----` マーカー配下にリストされた artifact が tracked 化されていれば `git rm --cached` で untrack
