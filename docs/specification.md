...
intent 系は Issue 側 `intent-label-issue-check.yml` ジョブが Issue ラベル付与数を機械的に検知（0 個 / 2 個以上で fail コメント）。PR には intent ラベルを付与しない（Issue #575 確定: intent の SoT は Issue ラベル）。レビュー判定（intent × severity）は `pr-fix` / `review-loop` が PR 本文から Issue 番号を解決して `gh issue view --json labels` で intent を直接取得する。
