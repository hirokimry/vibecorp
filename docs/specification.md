...
| **full** | AI 企業・コンプライアンス重視 | **ANTHROPIC_API_KEY 従量課金に到達しうる** | 自律実行 skills（`/autopilot` / `/spike-loop` / `/diagnose`） / C-suite 全員 + 分析員（14 ロール） |

- 上位プリセットは下位プリセットの機能を全て含む（加算モデル）
- **full プリセット**: ヘッドレス Claude を並列起動するスキル（`/autopilot`, `/spike-loop`, `/ship-parallel`）が含まれ、親プロセスの認証が `ANTHROPIC_API_KEY` の場合は API 従量課金に到達しうる。課金モデル詳細は [`docs/cost-analysis.md`](./cost-analysis.md) を参照
