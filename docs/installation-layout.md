...
`install.sh` を実行すると、導入先リポジトリに以下の構造が生成される。`README.md` には概要のみ残し、詳細は本ドキュメントに集約する。
...
| `lib/` | シェル共通関数（`common.sh` / `knowledge_buffer.sh` 等） | minimal 以上 |
...
| `ai-review.yml` | `claude_action.enabled: true` 時のみ | claude-code-action による AI レビュー |
| `ai-review-golden-test.yml` | `claude_action.enabled: true` 時のみ | claude-code-action のレビュー品質リグレッション検知 |
...
C*O / SM は全員 `decisions-index.md`（目次）+ `decisions/YYYY-QN.md`（四半期アーカイブ）の 2 段構成で判断を記録する。詳細は [`docs/migration-decisions-index.md`](migration-decisions-index.md) 参照。

`.claude/knowledge/{role}/decisions/` および `{role}/audit-log/` への作業ブランチ直書きは、Edit/Write 層 + Bash 層の 2 つの PreToolUse hook で deny される。書込みは `knowledge/buffer` worktree 経由のみ許可。詳細は [`docs/SECURITY.md`](SECURITY.md) 参照。

## XDG cache（.claude/ 配下に置かない揮発データ）
