...
> Issue #439 のガードレール導入後に取り残された `.claude/knowledge/` 差分を、機密情報スキャン込みで安全に main へ反映できるようになる。
> 旧 buffer worktree 構造・audit-log 旧構造・cycle-metrics 旧パスからの移行手順も網羅する。

# 📦 作業ブランチに残された knowledge 差分の救済手順

Issue #439 のガードレール導入後、作業ブランチに残ってしまった `.claude/knowledge/` 差分の救済手順を定める。

- 対象は `.claude/knowledge/{role}/decisions/` や `{role}/audit-log/` の差分。
- これらを `knowledge/buffer` 経由で main に反映する。

> [!NOTE]
> Issue #442 で監査ログ構造が `audit-*.md` フラット → `{role}/audit-log/YYYY-QN.md` 四半期集約に変更された。
> 新構造では `protect-knowledge-direct-writes.sh` が `.claude/knowledge/*/audit-log/*.md`（glob）を deny 対象に含む。
...
それ以前に作成された buffer worktree は新コードが期待する場所と整合しなかった。
...
旧構造 `accounting/audit-YYYY-MM-DD.md` フラット直置き → 新構造 `accounting/audit-log/YYYY-QN.md` 四半期集約に変更した。
...
> [!WARNING]
> バリエーション禁止。
> 半角カッコ（`(`）、別語句（`フォールバック判断記録`）、見出しレベル変更（`##`）等は全て検知漏れとなる。
...
| 観点 | 意図 |
|---|---|
| **無言データロスト回避** | BUFFER_DIR 取得失敗時にエージェントが判断記録を捨てるのではなく、ヘッダー付きで返すことで人間が必ず気づけるようになっている |
