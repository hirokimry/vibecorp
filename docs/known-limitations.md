...
teammate（並列 ship の Agent）が `/vibecorp:ship --worktree <path>` で起動された worktree 内で **素の `git commit` を直接呼んだ場合**、フックは main repo の cwd を見て `base_branch`（main 等）と判定し、コミットを deny する可能性がある。
...
`/vibecorp:commit` スキルは内部で `cd <path> && git commit` 形式を使うため、スキル経由なら本制限の影響を受けない（`rules/use-skills.md` でスキル経由が推奨されている理由の一つ）。
