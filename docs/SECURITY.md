...
| process-exec ホワイトリスト制限済み（Issue #513） | `(allow process-exec)` をホワイトリスト方式に変更し、`/usr`, `/bin`, `/sbin`, `/System`, `/Library`, `/opt/homebrew`, `/usr/local`, `/Applications`, `~/.local/share/claude` のみ実行可能。**WORKTREE 内に書き込まれた任意バイナリは実行不可**（攻撃者が悪意あるバイナリを write しても exec できない）。ただし osascript 等のシステムバイナリ自体を経由した境界迂回は依然として可能 |
