...
### AI レビュー（vibehawk）の認証経路

AI レビューの認証経路は
[`docs/ai-review-auth.md`](ai-review-auth.md) を Source of Truth とする。

> [!NOTE]
> レビューは Issue #531 で claude-code-action（vibecorp 自前ワークフロー）から **vibehawk へ移譲** された。
> 導入は `npx vibehawk setup`（GitHub App 作成 + 3 secrets 登録 + workflow 配布）で行う。
> claude-code-action 時代の OAuth 認証設計は同ドキュメントに **歴史的記録** として保持される。

以下は同ドキュメントに記載する。

- GitHub App（`VIBEHAWK_APP_ID` / `VIBEHAWK_PRIVATE_KEY`）+ Claude Max OAuth（`CLAUDE_CODE_OAUTH_TOKEN`）+ secrets 運用
- 漏洩時の revocation 手順
- Forked PR では secrets が渡らない仕様
- `CLAUDE_CODE_OAUTH_TOKEN` 空文字列による silent fail 防止 preflight（Issue #509、claude-code-action 時代の記録）
