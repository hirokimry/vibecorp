...
| 制約 | 内容 |
|------|------|
| `.claude.json.tmp.<pid>.<ms>` 動的サイドカー bind 不可 | bwrap は regex 許可をサポートしないため、起動時点で存在しない動的ファイルは bind できない。Claude Code の OAuth 原子的置換（rename）の挙動次第。CISO メタレビュー（2026-04-17 #329 / 2026-05-16 #310 マージ承認）では「rename 失敗時の挙動次第で攻撃面は既存と同等以下」と評価。Phase 2.1（#578）として実機検証手順を [「Linux Phase 2.1 — `.claude.json.tmp.<pid>.<ms>` 動的サイドカー実機検証手順」](#linux-phase-21--claudejsontmppidms-動的サイドカー実機検証手順) に明文化済み。検証実施は CEO ローカル / 自前ホストランナーで順次実施し、結果を本ドキュメントに追記する |
...
| **GitHub Actions `ubuntu-latest` (Ubuntu 24.04) では bwrap 起動不可** | AppArmor 4.0 の `unprivileged_userns` 制限により `bwrap` は `setting up uid map: Permission denied` で失敗する。CI 上の Linux 隔離テスト（`test_isolation_linux.sh` / `test_isolation_parity.sh`）は自動 skip され、実機検証は CEO ローカル環境または自前ホストランナーで実施する。`test.yml` の bwrap 試し実行ステップは failure ではなく `::warning::` を出して継続する |

#### Linux Phase 2.1 — `.claude.json.tmp.<pid>.<ms>` 動的サイドカー実機検証手順

CISO メタレビュー（2026-05-16）で Phase 2 マージ承認とともに「rename 失敗時の Claude Code 挙動は実機検証で確定する」と判定された未確認領域を埋めるための再現手順。CEO ローカルまたは自前ホストランナーで Phase 2.1（#578）として順次実施する。

**検証目的**: Claude Code が OAuth トークン更新時に `~/.claude.json.tmp.<pid>.<ms>` への write → rename を試みた際、bwrap が tmp サイドカーを bind していないことで rename が失敗した場合の Claude Code 側の挙動を以下 4 分類で測定する。

| 挙動 | 信頼境界への影響 |
|------|---------|
| (A) クラッシュ | 機能毀損あり。再ログイン不能なら Phase 2 を draft に戻して再設計（regex 許可サポートのある別 backend 検討等） |
| (B) ハング | UI 操作不能。最小実害は OAuth セッション 1 回分。CEO は SIGINT → 通常 claude（非 sandbox）で復旧可能 |
| (C) エラー継続 / 再試行 | sandbox 利用は実用上問題なし。エラーメッセージから根本原因が分かれば運用ドキュメントに追記 |
...
- Claude Code が npm/curl で正規インストールされていて、対話的 OAuth 認証経路が利用可能であること
...
| 観測結果 | 分類 | 対応 |
|---------|------|------|
| rename 成功 + ~/.claude.json 更新 | (D) fallback または bwrap が tmp 経由を透過 | 既存と同等以下と確定。docs/SECURITY.md にステータス追記 |
| rename EROFS/EACCES + ~/.claude.json 更新 | (D) Claude Code が tmp 失敗を catch して直接 write fallback | 同上 |
| rename EROFS/EACCES + ~/.claude.json 未更新 + プロセス継続 | (C) エラー継続 | 既知制約に挙動を追記、ユーザー向けエラーメッセージ品質を Phase 2.2 で改善検討 |
| rename EROFS/EACCES + プロセスハング | (B) ハング | docs/SECURITY.md の Linux 診断手段に SIGINT 復旧手順を追記。実用上は問題なし |
| rename EROFS/EACCES + プロセス終了（再起動不能） | (A) クラッシュ | **ロールバック発火**。Phase 2 を draft に戻し、再設計（regex 許可サポートのある別 sandbox backend 検討等） |

**結果記録先**:

- 検証結果サマリ: 本ドキュメント（docs/SECURITY.md）の Linux 固有制約テーブルから本セクションへリンクして反映
- CISO 判断履歴: `.claude/knowledge/ciso/decisions/YYYY-QN.md`（実施回ごとの一行記録）
- 監査ログ: `.claude/knowledge/security/audit-log/YYYY-QN.md`（Phase 2.1 検証実施ステータスを追記）

**現時点ステータス（2026-05-16）**: 未実施。本セクション追加と `bwrap-args.sh` コメント整理までを #578 PR で完了させ、実機検証は CEO ローカル / 自前ホストランナーで Phase 2.1 後続として順次実施する。検証実施後は本セクション末尾に「検証結果（YYYY-MM-DD）」サブセクションを追記する。
