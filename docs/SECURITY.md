...
| 2 | **プロンプトインジェクション経由の破壊** | 外部ドキュメント・Issue 本文・MCP レスポンスに仕込まれた指示で AI が `rm -rf ~` 等を実行 | `block-api-bypass.sh` フックで Bash 実行前に高リスクパターンを検出 | full プリセットの隔離レイヤ（macOS: `sandbox-exec` で WORKTREE 外への書込・`~/.ssh` 等の機密ディレクトリを deny。Phase 1 既知制約として `~/.claude` は全 RW 許可（後述「Phase 1 の位置づけと既知の制約」参照）。Linux: `bwrap` で同等の制限。Phase 1 は macOS 実装、Linux は Phase 2） |
