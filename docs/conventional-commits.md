...
> [!IMPORTANT]
> **intent ラベル → CC prefix の主従関係は絶対条件**。
> 逆引き（CC prefix → intent）は禁止。
> 1 つの Issue には intent ラベルを **1 つだけ** 付与する。

## 👥 読者像

このドキュメントは以下の読者向け。

- 💻 **開発者**: vibecorp のコミット規約と intent ラベルの主従関係を理解したい
- 🛠️ **利用者**: Issue / PR / commit のタイトル形式を vibecorp に揃えたい
...
判定フロー: **intent → CC prefix の順で決める**。

逆引き（CC prefix → intent）は行わない。
...
1. **起票・コミット時の判定フローは必ず `intent → prefix` の順で行う**
   ...
   - 次に対応する CC prefix を選ぶ。
     - CC 標準 11 種の中から、その intent に対応するものを選ぶ。
   - **逆順（prefix から intent を決める）は禁止**。
2. **「prefix が複数 intent を持つ」という見方は禁止**
   - 「intent に対応する prefix が複数ある」が正しい。
   - 例: `intent/security` に対応するのは `fix` / `feat` / `chore`。
     - intent から見た複数。
...
過去の議論で「`security:` 単独 prefix を採用しない」と判断した経緯がある。

- 典型例: CC 標準（業界都合）を主軸に置き、vibecorp 独自要件 `intent/security` を見失いそうになった。
- 着地: CC 拡張に飛びつかずに `intent/security` を主軸に据え直し、CC は `fix(security): ...` 等の scope 表記で吸収。

**prefix 主導で議論しそうになったら本セクションに立ち戻る**。

## 🧱 CC 11 種の vibecorp 厳格定義

各 prefix の vibecorp 厳格解釈と絵文字対応。
...
| vibecorp 厳格定義 | **観測可能な挙動が新たに加わる変更**。新規スキル / 新規フック / 新規エンドポイントなど |
| 注記 | 新機能追加が伴わない場合は使わない |
| 絵文字 | ✨ |
...
| 特に厳格化 | **テスト追加時に本番コードのバグも修正する場合は別 commit に分ける**（テスト追加 = `test`、本番コード修正 = `fix` の二段階） |
| 絵文字 | 🧪 |
...
> [!WARNING]
> 🔴 **厳格化大**。ランタイム挙動が変わるビルド変更は `build` 不可。
...
intent ラベル（主）から CC prefix（従）を選ぶ際の正引き表。
...
- 1 つの Issue には intent ラベルを **1 つだけ** 付与する。
  - PR には付与しない（CC prefix で機械可読を担保）。
- 複数 intent にまたがる変更は Issue を分割する。
- 機械的強制は Issue 側配布 workflow が行う。
  - **0 件または 2 件以上で fail コメント**: `templates/.github/workflows/intent-label-issue-check.yml`。

### 🔄 Issue #575 確定: intent SoT の一元化

intent の SoT は Issue ラベルに一元化された。

- PR は CC prefix（タイトル）が機械可読保険となる。
- レビュー判定（intent × severity）は `pr-fix` / `review-loop` が Issue 番号を解決し、`gh issue view --json labels` で intent を直接取得する。

#### 4 段フォールバックで Issue 番号を解決
