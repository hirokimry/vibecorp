...
- **CEO は代表取締役社長＝経営者である**：ソフトウェアが何をできるようになったか／壊れたか／変わるかという「ふるまい」で意思決定する。関数名・ファイルパス・diff のような実装詳細で判断するのではない。したがってエージェントから CEO への報告は全て動作主語で行う（具体規約は `.claude/rules/communication.md`）
...
| ロール | 役割 | docs/ 編集権限 |
|--------|------|---------------|
| SM（Scrum Master） | プロセス管理・進捗把握・エージェント間調整・並列判定 | `docs/ai-organization.md`（組織運営ドキュメントのメンテナンスのみ） |
...
各エージェントは Claude Code の subagent として独立した tool 呼び出しを行う。tools 宣言は frontmatter で固定する（Issue #448 で「Edit/Write 経由の書込みを誘導」する目的で統一）。
...
C\*O は `Edit / Write / MultiEdit` を持ち、knowledge への書込みは Edit/Write 層 hook（`protect-knowledge-direct-writes.sh`）が確実に検知できる。Bash redirect で書き込もうとした場合は Bash 層 hook（`protect-knowledge-bash-writes.sh`）が deny するため、tools 選択の誤りで素通りする経路はない（Issue #448 多層防御）。詳細は [`docs/SECURITY.md` の「knowledge ガードレール（多層防御）」](SECURITY.md#knowledge-ガードレール多層防御) を参照。
