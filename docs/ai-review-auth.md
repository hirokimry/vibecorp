...
>
> どちらか一方が欠けると claude-code-action は起動失敗する（`Could not fetch an OIDC token` エラー等）。`id-token: write` の permissions 設定は `templates/.github/workflows/ai-review.yml` で配布済み。
...
> **注**: `gh secret list` は secret 名のみを返し、値を取得できない。secret 名が登録されていても値が空文字列のケースは install.sh では検出不可能。このパターンは `ai-review.yml` の preflight ガード（後述「8. claude-review ジョブの preflight ガード」）が PR コメントで動的に通知する。
...
## 7. dismiss の責任分担（#466 確定）

レビューにおける 2 種類の dismiss は責任主体を明確に分離する。重複・越権を避けるため:

| dismiss 種別 | 責任主体 | 対象範囲 | タイミング |
|------------|--------|--------|---------|
| **approve dismiss**（review approval） | GitHub Branch Protection の `dismiss_stale_reviews: true`（#463 確定） | push 毎に **全レビュアーの approve** を一括 dismiss | push が来たタイミング |
| **review thread dismiss**（インラインコメント） | claude-action 自身が REVIEW.md の指示に従って実行（#466 確定） | **claude-action 自身のコメントのみ**、修正済み判定したもの | 再レビュー時、修正完了を確認したコメント単位 |

CodeRabbit / 人間レビュアーのインラインコメントは claude-action が触らない（越権行為禁止）。CodeRabbit 側は `auto_resolve.enabled: true` で同様の自走運用、人間コメントは作成者が手動で resolve する。

## 8. claude-review ジョブの preflight ガード

`claude-review` ジョブは `anthropics/claude-code-action@v1` を呼ぶ前に preflight ステップで `CLAUDE_CODE_OAUTH_TOKEN` の値が空でないことを検査する。空文字列のまま claude-code-action を呼ぶと内部の validation で `Either ANTHROPIC_API_KEY or CLAUDE_CODE_OAUTH_TOKEN is required` のエラー (exit 1) で死亡し、required check 未登録のため auto-merge を妨げず **AI レビュー pipeline が静かに死ぬ問題** が PR #508 で発覚した（Issue #509）。
...
| 警告コメント | 空のときのみ PR に投稿。文言は `⚠️ AI レビュー（claude-code-action）が起動できません` で始まる |
| 重複防止 | `gh pr view --json comments --jq` で既存の同種コメントを検出し、あれば再投稿しない |
| `gh` 引数 | workdir が空（checkout 前）のため `gh pr view` / `gh pr comment` には `--repo "$REPO"` を必須で明示する |
| 終了挙動 | 空判定にヒット → `exit 1`。後続の claude-code-action 呼び出しに到達しない |

### fail-safe 設計

`existing=$(gh pr view ...)` が GitHub Actions の `bash --noprofile --norc -e -o pipefail` 環境で失敗した場合、preflight step そのものが exit 1 で落ちる。コメント投稿には到達せず後続の claude-code-action 呼び出しも走らない。これは **意図的な fail-safe**: shell.md の「コマンドそのまま実行」ルールに従い `|| echo "0"` のフォールバックは使わない。preflight は「真のガード」であり、コメント投稿失敗を握りつぶして claude-action を起動させる方が危険。

### 文言変更時の注意

警告コメントの prefix（`⚠️ AI レビュー（claude-code-action）が起動できません`）は重複検出用キーとして使われる。変更する際は以下 4 箇所を **同時に** 更新する必要がある:
...
| `tests/test_ai_review_preflight.sh` | 動的（bash 実行） | 空 token + 既存コメントなし → comment 1 回 + exit 1 / 空 token + 既存コメントあり → comment 0 回 + exit 1 / 設定済み token → exit 0 |
