...
vibecorp 運用に必要な定額サブスクリプションを以下に列挙する。`ANTHROPIC_API_KEY` を併用する場合は本表に含めず、後述「変動費 > API 利用コスト」および「実行モード別の課金モデル」を参照する。
...
| サービス | 単価 | 想定月間量 | 月額見積 |
|----------|------|-----------|----------|
| Claude Opus 4.6 | 入力 $15 / 出力 $75 per 1M token | `/vibecorp:diagnose` 用、24h cadence 7 サイクル/週で約 4.2M/月（固定費 600K/サイクル × 7 × 約 4 週） | 約 $63/月（出力分含む参考値） |
| Claude Sonnet 4.6 | 入力 $3 / 出力 $15 per 1M token | `/vibecorp:ship` 系・合議制分析員用（メイン消費）。24h cadence 満枠で月間消費の大半を占める（合計 約 90M/月、cost-principles.md L91） | 約 $770/月（24h 満枠想定） |
| Claude Haiku 4.5 | 入力 $1 / 出力 $5 per 1M token | 定型・軽量タスク用（現状ほぼ未割当、将来エージェント拡張用） | 約 $0/月（現状） |

月額総計の目安: **24h cadence 満枠時 約 $840/月**（cost-principles.md L121 警告閾値 $1,200 の手前。後述「自律実行の上限ガードレール」の試算表も参照）。
...
- **GitHub Actions minutes**: Public リポジトリは課金なし。Private は月 2,000 minutes 無料枠 + 超過分は [GitHub Actions billing](https://docs.github.com/en/billing/managing-billing-for-your-products/managing-billing-for-github-actions) を参照。確認手順は後述「GitHub Actions minutes の手動確認方法」を参照
...
| Phase 2（小規模チーム運用） | 3-5 名 | 24h cadence 満枠 90M + チーム PR レビュー追加 30-60M | $200〜$400 | Claude Max 1 + 必要時 ANTHROPIC_API_KEY フォールバック（線形モデルで追加 30-60M ≒ Sonnet 換算 約 $90-280） |
...
注: Claude Max 定額環境では金銭限界費用ゼロ、**レート制限（5 時間あたりの token 上限）が制約軸** となる（cost-principles.md L20）。Phase 3 規模では Bot 専用 Max 別契約を推奨（後述「Bot 経由 Claude Max OAuth のレート枯渇リスク」を参照）。
...
- 監査閾値の運用詳細（「100% 到達時」の解釈・手動停止フロー）は後述「monthly cost cap 運用方針」を参照
...
claude-code-action の Bot 認証に **リポジトリ管理者個人の Claude Max OAuth トークン**（`CLAUDE_CODE_OAUTH_TOKEN`）を使う場合、対話用クォータが枯渇するリスクがある。Anthropic 公式が個人 OAuth の自動化用途利用に対して明示的に警告している事項であり、本ドキュメントに転記する。
...
| 対策 | 内容 | 推奨度 |
|---|---|---|
| cadence 伸長 | `/vibecorp:autopilot` の定期実行を 24 時間 → **36 時間**に伸ばす（短時間集中時の負荷を分散） | ✅ 推奨 |
| Bot 専用 Max 別契約 | Bot 用メールアドレスで別途 Claude Max を契約し、その OAuth を `CLAUDE_CODE_OAUTH_TOKEN` に登録する | 🟢 将来選択肢 |
| API 従量課金切替 | 高頻度運用時は `ANTHROPIC_API_KEY` で API 従量課金に切り替える（コスト発生に注意） | ⚠️ コスト要試算 |

### 安心材料

- **Forked PR は claude-action 対象外**（secrets 不在で自動スキップ）。外部コントリビューターの PR ではクォータ消費は起きない
