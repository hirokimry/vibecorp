...
デフォルト構成は **vibehawk-only**（`vibehawk.enabled: true` / `coderabbit.enabled: false`、Issue #531 / #783）。vibehawk は `CLAUDE_CODE_OAUTH_TOKEN` 経由で動くため別途サブスクリプションは不要（Claude Max 枠を共有する。詳細は後述「Bot 経由 Claude Max OAuth のレート枯渇リスク」）。CodeRabbit は任意トグルで、`coderabbit.enabled: true` にした場合のみ下表の CodeRabbit 行が発生する。

| 項目 | 月額 | 年額 | 備考 |
|---|---|---|---|
| Claude Code Max 20 プラン | $200 | $2,400 | `ANTHROPIC_API_KEY` 未設定時の定額運用基準。vibehawk レビューも同枠を共有 |
| CodeRabbit Free（`coderabbit.enabled: true` 時のみ） | $0 | $0 | 任意トグル。3 reviews/hour 制限。詳細は [CodeRabbit Pricing](https://www.coderabbit.ai/pricing) を参照 |
...
出典: [.claude/knowledge/accounting/cost-principles.md](../.claude/knowledge/accounting/cost-principles.md) §固定費
...
出典: [.claude/knowledge/accounting/cost-principles.md](../.claude/knowledge/accounting/cost-principles.md) §変動費
...
出典: [.claude/knowledge/accounting/cost-principles.md](../.claude/knowledge/accounting/cost-principles.md) §スケール時の閾値（後述「自律実行の上限ガードレール」の試算表も参照）
...
出典: [.claude/knowledge/accounting/cost-principles.md](../.claude/knowledge/accounting/cost-principles.md) §コスト指標の要注意・警告閾値
...
vibehawk の Bot 認証で **リポジトリ管理者個人の Claude Max OAuth トークン**を使う場合がある。
...
> [!NOTE]
> レビューは Issue #531 で claude-code-action から vibehawk へ移譲されたが、**負担構造は不変**。
> vibehawk も同じ `CLAUDE_CODE_OAUTH_TOKEN`（個人 Max OAuth）経由でレビューを実行するため、本節の枯渇リスク・共有負担・緩和策はそのまま適用される。
...
ローカルレビューも vibehawk の Bot レビューと同じレート枯渇リスクに晒される。
...
- 📌 注記: 本セクションの cadence 36h 推奨は以下に適用される。
  - 適用条件: `CLAUDE_CODE_OAUTH_TOKEN`（個人 Max OAuth）で vibehawk の Bot レビューを運用する場合。
  - Bot 専用 Max 別契約や `ANTHROPIC_API_KEY` 従量課金運用時は、次節のデフォルト 24h 推奨に従う。
  - 参照節: 「自律実行の上限ガードレール」。
- ✅ MUST: 個人 Max OAuth で vibehawk の Bot レビューを有効化する前に、本セクションの共有負担リスクをチームに周知すること。
...
- 🤝 **レビューツール側のデフォルトに任せる**。
  - PR ごとの `max_tokens` / 大規模 PR の閾値は vibehawk / CodeRabbit 側のデフォルト挙動に委ねる。
  - vibecorp 独自の閾値設定キーは追加しない。
- 🚫 **新規キー追加は行わない**。
  - レビュアー選択は `vibehawk.enabled` / `coderabbit.enabled` の独立トグル（Issue #531）のみで運用する。
  - 月次コスト上限・PR ごとの max_tokens を表現する新規キーは追加しない。
...
AI レビュー（vibehawk）は GitHub Actions 上で実行されるため、Actions minutes を消費する。
...
4. 想定を超えている場合、`.github/workflows/` の trigger 条件（特に `npx vibehawk setup` が配布する vibehawk レビューワークフローの `on:` 節）を見直す。
