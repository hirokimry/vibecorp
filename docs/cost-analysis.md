...
- **`vibecorp.yml` に新規キーを追加しない**: `claude_action.enabled` と `claude_action.skip_paths` の 2 キーのみで運用する（Issue #468 確定）。月次コスト上限・PR ごとの max_tokens を表現するキーは本リリースでは追加しない
- **GitHub Actions minutes 監視機構も持たない**: GitHub Settings → Billing で利用者が手動確認する（後述）

### Claude Max 定額環境での実質上限（90M token/月）

`Claude Max 20`（$200/月）定額プランで `/vibecorp:autopilot` を運用する場合、**月間トークン消費 約 90M token を目安**とする。これは Issue #455 の CFO 条件付き承認における目安値であり、レート制限（5 時間あたりのトークン上限）への接触余裕を確保する基準である。

| 指標 | 目安 |
|------|------|
| 月間トークン消費上限（Claude Max 20 定額） | **約 90M token/月** |
| 24 時間ごと運用での 1 日あたり消費 | 約 3M token/日 |
| 推奨運用 cadence | 24 時間ごと（`/loop /vibecorp:autopilot 24h`） |

90M token/月の目安値は Issue #475（実機検証期間）で実測補正する。Claude Max 定額内で動く限り追加課金は発生しないが、レート制限に到達した場合は `ANTHROPIC_API_KEY` への自動フォールバックが働く可能性があるため、Anthropic Console での使用量モニタリングを必ず併用すること（次節）。
...
3. `Monthly spend limit` を任意の USD 額で設定する（例: $100）
...
| `ANTHROPIC_API_KEY` 併用（Max フォールバック含む） | 月額予算の 80% / 100% アラート必須 |
| API Key のみ運用（Max なし） | 月額予算の 50% / 80% / 100% アラート推奨 |
