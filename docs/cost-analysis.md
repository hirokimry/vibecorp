...
- **`max_issues_per_day`: 14** — `max_issues_per_run` × 2（cadence=24h を 2 サイクルに分割した想定上限値）に整合させた値。**推奨運用 cadence は 24 時間ごと**（旧 12 時間ごと運用ではレート制限が逼迫するため非推奨）。満枠運用時の月額概算は次節の試算表を参照
