...
- 同期対象: `severity/coderabbit.md`（CodeRabbit 公式仕様の記録）と定義を完全一致させる。
...
`severity/coderabbit.md`（CodeRabbit 公式仕様の記録）と本ファイルの定義は **完全に一致** させる。
...
- `claude_action.enabled: false` 運用（例: vibecorp 本体の現状）では `REVIEW.md` 自体が配布されないため、本ファイルの severity 定義は CodeRabbit 側との同期記録として保持される（`severity/coderabbit.md` と同じ役割）。
