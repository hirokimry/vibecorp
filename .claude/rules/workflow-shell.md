GitHub Actions の workflow yaml には 3 行以上のシェル処理を書かず、`.github/scripts/` 配下のスクリプトに切り出して呼ぶこと。
...
リポジトリには既に `scripts/` 配下にスクリプトが置かれている場合がある（vibecorp 本体では `scripts/backfill-intent-labels.sh` 等）。本ルール新設後の **新規切り出しは `.github/scripts/` を使う**。既存 `scripts/` 配下スクリプトの移動は段階移行とし、別 Issue で扱う。
