...
## 2026-05-16 — CISOセッション知見 — Link Following 脆弱性、合議分裂運用、audit-log 追記漏れ検出

### 知見1: Link Following 脆弱性は TOCTOU と独立した攻撃面

`source "$BWRAP_ARGS_FILE"` 等で外部制御可能なパスを扱う際、`-f` のみチェックして `-L` チェックを欠かすと symlink 経由の Link Following 攻撃を防げない。

- `-f` は symlink 先が実在すれば `true` を返す（symlink チェックをバイパスできる）
- TOCTOU（チェック後のファイル内容差し替え）とは独立した別の攻撃ベクタ
- `docs/SECURITY.md` の既知制約欄が「TOCTOU」のみ記載しているケースでも Link Following は別問題として扱う必要がある

**必須チェックパターン**:

```bash
if [[ ! -f "$FILE" || -L "$FILE" ]]; then exit 1; fi
```

この知見は `security-principles.md` の「Link Following 脆弱性と TOCTOU は独立した攻撃面」セクションに詳細を記載。

### 知見2: security-analyst 3並列合議の分裂時運用

security-analyst を 3 並列起動した結果が H-1 / Medium / Medium のように分裂した場合の CISO メタレビュー判断基準:

| 状況 | 判断 |
|------|------|
| 1 名でも High / Critical を検出 | 全会一致ルール適用。差し戻し |
| 複数 analyst が同一箇所を独立指摘（共通指摘） | 採用優先（偽陽性リスク低） |
| 1 analyst のみの指摘 | 攻撃チェーン観点で慎重に偽陽性評価 |
| 全員 Low / Info のみ | 全会一致ルール不発動。CISO 独立評価で判定 |

severity 呼称のゆれ（「H-1」「High」「Critical」等）は文脈から deep severity に読み替える。呼称統一より実質的な判定を優先する。

### 知見3: CISO decisions-index.md への追記漏れ検出

`docs/SECURITY.md` に「CISO メタレビュー YYYY-MM-DD」を記録した PR で `.claude/knowledge/ciso/decisions/` への追記が漏れるパターンがある。sync-check フェーズで以下を機械的に検出できる:

- SECURITY.md の更新日と decisions-index.md の最終エントリ日付の乖離
- decisions-index.md に `（エントリはまだありません）` または空索引が残存

検出したら decisions/ アーカイブへの追記を促し、decisions-index.md を補完する。

### 対応状況

- `security-principles.md` に Link Following 脆弱性の知見を追記（2026-05-16）
- 本エントリで audit-log に記録
