...
## 2026-04-25 — Issue #401 Plugin 名前空間 Phase 3: 互換スタブ廃止（セキュリティレビュー）

### 対象範囲

- `install.sh`（`--update` モードで `vibecorp.lock` の `plugin_skills` リストに基づく `.claude/skills/` 配下ファイル削除）

### 変更概要

`vibecorp.lock` の `plugin_skills` リストに記載されたスキルスタブファイルを `rm -rf` で削除する処理を `install.sh --update` に追加。削除パスは `${skills_dir:?}/${name:?}` で構成。

### セキュリティ対策の評価

| 対策 | 実装 | 評価 |
|------|------|------|
| 空変数ガード | `${skills_dir:?}` / `${name:?}` による `:?` 展開 | 適切。未設定時は即 exit し `rm -rf /` 等を防止 |
| パストラバーサル防止 | `[[ "$name" == */* ]]` でスラッシュ含む名前を除外 | 適切。`../` を含む相対パス上昇を遮断 |
| 削除対象の範囲限定 | `vibecorp.lock` の `plugin_skills` リストのみ（ユーザー編集不可の管理ファイル） | 適切。外部入力直接使用ではなくロックファイル経由 |

### 残留リスク

- `vibecorp.lock` 自体が改ざんされた場合、`plugin_skills` に `../` 以外の方法（絶対パス等）で想定外パスを混入させる余地が理論上ある。ただし同ファイルは vibecorp 管理ファイルであり、改ざんには install.sh 本体を改ざんできる同等の権限が必要。現実的な攻撃面なし。
- `name` がディレクトリ名の場合、`rm -rf` でディレクトリごと削除される。`.claude/skills/` の直下ディレクトリが削除対象になりうるが、これは意図した仕様（スタブ廃止）の範囲内。

### 判定

**問題なし** — Critical / High / Medium 脆弱性は検出されなかった。`:?` ガードとスラッシュ除外の 2 段防御により、`rm -rf` の誤爆リスクは適切に制御されている。
