...
- **判断**: 境界・制約の詳細なパス一覧は実装ファイル（例: `claude.sb`）のヘッダコメントを正典とし、設計ドキュメント（`docs/design-philosophy.md`）では思想のみを記述してパス列挙を避ける
- **根拠**: 同じパス一覧を 2 箇所に書くと、実装を更新したときにドキュメント側の更新が漏れて乖離が発生する。「正典は 1 箇所」の原則（Single Source of Truth）を維持しつつ、設計ドキュメントには「詳細は当該ファイルを参照」と参照先を明記することで両方の役割を成立させる
- **適用範囲**: sandbox プロファイルの許可/拒否パス一覧に限らず、設定ファイル・テンプレートのキー一覧など「実装が正」になる詳細全般に適用できる汎用パターン
