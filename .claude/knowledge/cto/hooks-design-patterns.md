...
ゲート系フックはステートファイルで状態管理する。スタンプパスは XDG キャッシュ配下に配置する:

```text
${XDG_CACHE_HOME:-$HOME/.cache}/vibecorp/state/<repo-id>/{gate名}-ok
```

`<repo-id>` は `git rev-parse --show-toplevel` の basename に sha256 先頭8桁を付加して生成する（例: `vibecorp-a1b2c3d4`）。

- ステートの生成元: 対応するスキル（例: `/sync-check` が `<repo-id>/sync-ok` を生成）
- ステートの消費: ゲートフックがファイルの存在を確認し、確認後に `rm -f` で即座に削除する
- **ワンタイム性**: 1回の push / merge につき1回のスキル実行を強制する設計。ステートを使い回せない
- **リポジトリ分離**: リポジトリルートの SHA256 ハッシュにより、同一マシン上の複数リポジトリ・worktree を自動的に分離する
- **書き込み前のディレクトリ作成**: state ディレクトリが存在しない可能性があるため、生成側（スキル / フック）は `vibecorp_stamp_mkdir()` ヘルパーを使用する
- **ship-parallel との連携**: worktree は XDG キャッシュを共有しないため、main の state を持ち込まない

### 共通ヘルパー（`templates/claude/lib/common.sh`）

| 関数 | 役割 |
|------|------|
| `vibecorp_stamp_dir` | `<XDG_CACHE>/vibecorp/state/<repo-id>` を返す（mkdir は行わない） |
| `vibecorp_stamp_path <gate名>` | `<stamp_dir>/<gate名>-ok` のフルパスを返す |
| `vibecorp_stamp_mkdir` | stamp ディレクトリを `mkdir -p` して返す |

直接 `mkdir -p` を呼ばず、必ず `vibecorp_stamp_mkdir` を経由すること。
...
ログファイルは `$(vibecorp_stamp_mkdir)/command-log` に出力する:

- XDG キャッシュ配下（`${XDG_CACHE_HOME:-$HOME/.cache}/vibecorp/state/<repo-id>/`）に置くことで、リポジトリ・worktree ごとに SHA256 ハッシュで自動分離される
- キャッシュディレクトリはリポジトリルート外のため、`.gitignore` 設定に依存せずコミット対象にならない
- `vibecorp_stamp_mkdir()` が `mkdir -p` も行うため、別途ディレクトリ作成は不要
