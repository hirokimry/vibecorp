...
## 新しいセットアップ関数追加時のテスト影響

`install.sh` に新しいセットアップ関数（`setup_secrets` 等）を追加すると、非対話モード + 環境変数未設定でエラー終了するため、全ての full プリセットテストが連鎖して失敗する。

### 原因

full プリセットテストは `setup_mock_docker` のようなモックヘルパーを使って前提条件を満たしている。新しいセットアップ関数を追加した際にモックヘルパーを用意しないと、既存テスト全体が壊れる。

### 対処パターン

1. 新しいセットアップ関数（例: `setup_secrets()`）を追加したら、対応するモックヘルパー（例: `setup_mock_secrets()`）をテストファイルに追加する
2. full プリセットを使う全テストケース（C2, T1-T6, AI1, D5 等）に新しいモックヘルパーを適用する
3. テストファイルが大きいため追加漏れが起きやすい。`grep -n "setup_mock_docker"` で適用箇所を洗い出し、同じ箇所に漏れなく追加する

```bash
# 適用漏れを確認するコマンド例
grep -n "setup_mock_docker\|run_install_full" tests/test_install.sh | head -40
```
