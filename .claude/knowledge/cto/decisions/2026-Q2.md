...
## 2026-04-25 — Plugin 名前空間 Phase 3（#359）— ドキュメント整合性チェック

### 対象変更

Plugin 名前空間 Phase 3: `.claude/skills/` 互換スタブの廃止。

### チェック結果

**docs/design-philosophy.md — 矛盾あり**

3層アーキテクチャ図（78〜89行目）に以下の記述が残っている:

```
│   ├── skills/    ← 互換レイヤ（スタブ。/vibecorp:xxx への誘導）
```

Phase 3 完了により `.claude/skills/` は廃止済みであり、この行の存在自体が矛盾。同ファイルの97〜99行目には「互換レイヤの廃止（Phase 3 完了）」として正しく記載されているが、アーキテクチャ図は更新されていない。アーキテクチャ図から `.claude/skills/` 行を削除するか、廃止済みの旨を明示する修正が必要。

**docs/file-placement.md — OK**

`.claude/skills/` の記述は「実行層: ワークフロー定義（/コマンド）」として残っているが、19行目の記述内容を確認すると `← 実行層: ワークフロー定義（/コマンド）` として `.claude/skills/` が記載されている。ただし 123〜125行目の `### skills/ — ワークフロー定義（Plugin 名前空間）` セクションは Plugin ルートの `skills/` を正しく記述しており、ここは整合している。ディレクトリ構造の全体図（8〜26行目）の `.claude/skills/` 行は実態と乖離している点を要確認。

再確認: ファイル内の `.claude/skills/` 記載は全体図(19行目)のみ。こちらは廃止済みディレクトリを現役として記載しており矛盾。

**docs/migration/plugin-namespace.md — OK**

Phase 3 列を追加した3列テーブルで廃止を明記。移行手順も正確に反映されている。

**docs/preset-addition-checklist.md — OK**

テンプレートパスが `skills/新しいスキル/SKILL.md` と Plugin ルート形式に更新されており整合している。

**docs/ai-organization.md — OK**

Phase 3(#359) 完了追記は内容確認済み。

**.claude/knowledge/agents-vs-skills.md — OK**

`skills/` の記述が Plugin 名前空間形式に更新されている。

### 判断

- `docs/design-philosophy.md` の3層アーキテクチャ図の `.claude/skills/` 行と `docs/file-placement.md` の全体図の `.claude/skills/` 行は、Phase 3 完了後の実態と矛盾している。要更新。
- 他のドキュメントは整合している。

## 2026-04-25 — CodeRabbit 修正整合性チェック（docs/file-placement.md, docs/migration/plugin-namespace.md, install.sh）

### 対象変更
1. `docs/file-placement.md` L289: `.claude/agents/` を許容配置先に追記
2. `docs/migration/plugin-namespace.md` L30: `--update` クリーンアップ条件に `vibecorp.lock` が存在する場合を追記
3. `install.sh` L754: `$skills_dir` を無条件追加 → `[[ -d "$skills_dir" ]]` 存在チェック付きに変更

### 判断

3点の変更自体に新たな矛盾は確認されなかった。

- `install.sh` L754 の存在チェック追加は防御的で適切。
- `docs/migration/plugin-namespace.md` L30 の条件追記は実装の実態と一致。
- `docs/file-placement.md` L289 の `.claude/agents/` は「配置してはいけないもの」セクションの例外許容パスとして整合している。

残存懸念: 同一ファイル `docs/file-placement.md` に廃止済み `.claude/skills/` 行が残存している可能性（Issue #359 相当、2026-04-25 記録）。今回の修正で解消されているか別途確認が必要。


## 2026-04-25 — 本体リポジトリでの --plugin-dir 不要化の技術検討

### 背景
vibecorp 本体リポジトリ（開発元）では `claude --plugin-dir .` を明示しないと `/vibecorp:*` スキルが自動認識されない。導入先では `install.sh` が `.claude-plugin/plugin.json` + `skills/` を配置するため自動認識されている。

### 現状構造
- `.claude-plugin/plugin.json` はリポジトリルートに存在する（確認済み）
- `skills/` もリポジトリルートに存在する
- つまり構造は導入先と同一。自動認識されないのは Claude Code の plugin 自動検出ルールによる

### 技術選択肢と評価

**案A: `.claude/settings.json` に `pluginDirs` を追記**
- Claude Code が `pluginDirs` 設定を読んで自動で plugin-dir を追加するなら有効
- 現時点でこの設定キーが公式に存在するか未確認（claude --help / 公式ドキュメント要確認）
- リスク: 非公式キーを設定に書くとドキュメントと乖離する

**案B: CLAUDE.md に `claude --plugin-dir .` の起動手順を明記するだけ**
- 最も低コスト。本体開発者はフラグを知って使えばよい
- コード変更ゼロ。誤設定リスクなし
- 導入先との非対称性は残るが「本体開発者は慣れた開発者」という前提では許容範囲

**案C: shell alias / Makefile / direnv で自動付与**
- `direnv` で `CLAUDE_FLAGS="--plugin-dir ."` をセット、または `Makefile` の `dev:` ターゲットで起動
- 環境依存（direnv の有無）で再現性に差が出る
- CONTRIBUTING.md に記載すれば許容範囲

**案D: `install.sh --dev` モードで本体リポジトリにも自己インストール**
- 本体に `install.sh` を自己適用して `.claude-plugin/` を再配置する
- 二重管理になりソースと成果物が同一リポジトリに混在する。技術的負債

### 判断
**現時点での推奨は案B + 案C の組み合わせ**。CLAUDE.md または CONTRIBUTING.md に起動コマンドを明記し、任意で direnv を使う。案Aは公式サポートが確認できれば最善だが、確認なしに採用するのはコメントとコードの乖離リスクがある。案Dは却下。

Claude Code が `.claude-plugin/plugin.json` をルートに見つけても自動認識しない理由は、「`--plugin-dir` フラグ指定がない限り CWD の plugin.json を読まない」という Claude Code の仕様によるものと推定。公式仕様の確認が前提。


## 2026-04-25 — ~/.zshrc シェル関数による --plugin-dir 自動付与案のレビュー

### 対象

vibecorp 本体リポジトリで `claude` と打つだけで `--plugin-dir .` が付くよう `~/.zshrc` にラッパー関数を追加する案。

```bash
claude() {
  if [[ -f .claude-plugin/plugin.json ]]; then
    command claude --plugin-dir . "$@"
  else
    command claude "$@"
  fi
}
...
- **正確性**: 基本動作は正しい。`"$@"` による全引数透過も問題なし。サブディレクトリ作業時にプラグインが有効にならないエッジケースがあるが、実害は小さい
- **安全性**: `command claude` による再帰呼び出し回避は正しく実装されている
- **影響範囲**: 導入先で `--plugin-dir` が重複指定された場合の Claude Code の挙動（エラー/警告/黙殺）が公式未確認。要確認
- **保守性**: `~/.zshrc` への書き込みはバージョン管理対象外。Claude Code インターフェース変更時に静かに壊れるリスクがある

### 判断

**要注意 — 動作は正しいが採用前に2点を確認する**

1. Claude Code の `--plugin-dir` 重複指定時の挙動を確認する
2. CONTRIBUTING.md または CLAUDE.md に「本体開発者向け起動方法」として明記し、`~/.zshrc` 追記は必須要件ではなく任意の個人設定として位置付ける

過去判断（2026-04-25「本体リポジトリでの --plugin-dir 不要化の技術検討」）との整合: 今回の提案は案C（shell 関数）に相当。案B（CLAUDE.md 明記）と組み合わせる形が推奨。案C 単独の必須化は保守性の観点から推奨しない。
