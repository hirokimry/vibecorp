...
検証完了判定（A + B）を満たした後、**本番運用への切替** は以下で判定する（検証完了とは別の判断軸）:

- **C\*O 合議**: CFO + CISO + CTO + CPO の 4 役合議で OK 判定

完了判定（A + B）と切替判定（C\*O 合議）は段階を分離して判定すること。週次サマリ上では「検証完了」（A + B 充足の集計）まで記録し、4 役合議の結果は本番運用切替時に別途記録する（`docs/ai-review-dependency.md` の「C\*O 合議による本番運用切替」参照）。
