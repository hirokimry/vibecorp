...
## 2026-04-29 — 不可領域チェック: knowledge/ 配下の構成統一リファクタ（C*O/分析員/揮発データ三領域）
...
- `.claude/knowledge/` 配下の構成統一（cfo/cto/cpo/ciso/clo/sm/accounting/security/legal）
- 監査ログ（audit-log/）新設・append-only から四半期集約への移行
- 揮発データ（cycle-metrics）を `~/.cache/vibecorp/state/<repo-id>/` へ撤退
- `protect-knowledge-direct-writes.sh` の deny パターンを `audit-*.md` から `audit-log/` 系に変更（templates / 本体 同期必須）
- 各スキル（audit-cost / audit-security / cycle-metrics）と分析員エージェント定義の追従
- 既存テスト 5 ファイルの追従 + 新規 test_audit_log_structure.sh 追加

### 各不可領域の該当可否

| 領域 | 判定 | 根拠 |
|------|------|------|
| 1. 認証 | 非該当 | `*auth*.sh` / `*permission*.sh` / settings.json permissions セクション / gh auth / ANTHROPIC_API_KEY の変更なし |
| 2. 暗号 | 非該当 | encrypt/decrypt/secret/credential/token を扱うコードの変更なし |
| 3. 課金構造 | 非該当 | docs/cost-analysis.md / max_issues_per_day / claude -p 等の変更なし |
| 4. ガードレール | 精査要 → 非該当 | protect-knowledge-direct-writes.sh のパターン変更を含むが、protect-files.sh / diagnose-guard.sh / forbidden_targets / diagnose-active スタンプの制御ロジックは不変。変更はガードレール対象パスの精緻化（audit-*.md → audit-log/）であり、buffer 経由のみ許可する原則は維持される |
| 5. MVV | 非該当 | MVV.md の変更なし |

### 攻撃チェーン観点

`protect-knowledge-direct-writes.sh` のパターン変更が唯一の懸念点。変更前の `audit-*.md` パターンは audit-log/ ディレクトリ構成への移行後には機能しなくなる（対象ファイルが存在しなくなる）ため、パターンを `audit-log/` 系に更新しなければガードレールの保護が空振りになる。今回の変更はその整合性を担保するものであり、保護を弱める方向の変更ではない。

攻撃経路として考えられる「deny パターンの緩和による audit-log/ への直接書き込み」についても、変更後は `audit-log/` 系を明示的に deny 対象に加えることで保護強度は維持または向上する。

深刻度: 低（ガードレール対象パスの精緻化、保護の弱化なし）

### 過去判断との一貫性

2026-04-22「承認ダイアログのスマホ remote app 非表示問題」で protect-files.sh 承認フロー変更リスクを理由に除外した事例と区別する。今回は protect-files.sh 本体の変更ではなく、knowledge 書き込み専用の protect-knowledge-direct-writes.sh のパターン変更であり、スコープが異なる。一貫。
