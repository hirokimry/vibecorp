...
## 2026-04-14: `gh auth token` 自動取得方式のセキュリティ評価

### 判断内容

`install.sh` の `setup_secrets()` における GitHub トークン取得に `gh auth token` 自動取得（ホスト側 OAuth セッションからの抽出）を使用する方式を評価。

### 検出リスク

#### 主要問題: SECURITY.md 最低条件 5 番との矛盾

SECURITY.md は「fine-grained token で対象リポジトリ・対象操作のみに制限する」を最低条件として明記している。しかし `gh auth login`（ブラウザ OAuth）で取得されるトークンは classic OAuth トークン（`repo` / `read:org` / `workflow` 等の broad scope）であり、fine-grained token ではない。設計ドキュメントと実装が正面から矛盾する。

#### トークンスコープの不一致

- `gh auth token` が返すトークンはユーザーの全リポジトリに対して `repo` スコープを持つ可能性がある
- vibecorp が操作する対象リポジトリ 1 件に限定されない
- egress allowlist は「どこに通信するか」を制限するが「何を送るか」を制限しない。broad-scope トークンが渡っていれば、api.github.com への正規 API 呼び出しとして全リポジトリ操作が可能になる

#### ファイル永続化による生存期間の延長

- `gh auth token` は gh の OAuth セッションが生きている限り有効
- `secrets/github_token` に書き出した後は、gh セッションを削除してもファイルが残存する
- バックアップ・同期ツールによる二次流出リスク

### 攻撃チェーン（最悪シナリオ）

```text
プロンプトインジェクション
→ コンテナ内で任意コマンド実行（dangerously-skip-permissions により確実）
→ /run/secrets/github_token から broad-scope OAuth トークンを読み取り
→ egress allowlist 内の api.github.com 経由で正規 API 呼び出し形式で悪用
→ トークンスコープ内の全リポジトリに対して read/write 操作
```

fine-grained token であれば被害範囲は対象リポジトリ 1 件。broad-scope トークンの場合はユーザーの全リポジトリに拡大する。

### 最終判断

**差し戻し（部分的）。`gh auth token` 自動取得をデフォルト動作から除去すること。**

#### 修正内容

1. `install.sh` の `setup_secrets()` から `gh auth token` 取得経路を削除（451 行目）
   - 変更後の優先順: 既存ファイル → 環境変数 → 対話入力（fine-grained token 推奨案内付き） → エラー終了
2. `docker/claude-sandbox/README.md` の「`gh auth login` 済みなら `gh auth token` から自動取得されるため、手動でのトークン配置は通常不要」という記述を削除または修正
3. `docs/SECURITY.md` のデータ保護セクションを修正し、fine-grained token の発行を必須とする記述に更新
