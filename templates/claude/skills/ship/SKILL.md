...
単体 `/ship` を直接呼んだ場合もコンテナ内で実行する。worktree モード（`--worktree <path>`）かつ `VIBECORP_IN_CONTAINER=1` が設定されている場合は、既にコンテナ内で実行中のためコンテナ起動をスキップする。

### コンテナ前提条件チェック

worktree モードでない場合、ワークフロー開始前に以下を確認する:
...
ブランチ作成（ステップ 1）完了後、以下でコンテナを起動する:
