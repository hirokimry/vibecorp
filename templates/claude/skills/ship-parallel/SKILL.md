...
作業を開始する前に、以下のコマンドを実行してワークツリー分離を検証してください:

1. 現在の作業ディレクトリを確認:
   pwd

2. git worktree であることを確認:
   git rev-parse --show-toplevel

3. メインリポジトリと異なることを確認:
   - 作業ディレクトリに `.claude/worktrees/` が含まれていること
   - または、メインリポジトリのパスと異なること

以下のいずれかに該当する場合、**即座に作業を中断**し、エラーを報告してください:
- 作業ディレクトリがメインリポジトリと同一である
- `.claude/worktrees/` 配下ではない
- git worktree list でメインリポジトリの1行しか表示されない
...
**最初の Agent 起動直後に、スキル実行者（オーケストレーター）自身が分離を検証する。**
...
以下を確認する:
- `git worktree list` の出力が **2行以上** であること（メイン + Agent worktree）
...
**検証失敗時**: 全 Agent にシャットダウンリクエストを送信し、ユーザーに報告して即座に中断する。ワークツリー分離が機能していない状態での作業続行は禁止。
...
| ワークツリー分離が確認できない | ステップ 5d |
| 個別の /ship が介入ポイントに到達 | ステップ 5 |
| 並列 PR 間でコンフリクト検出 | ステップ 7 |

## 制約

- **ワークツリー分離が確認できない状態での作業続行は絶対に禁止** — `git worktree list` で分離を確認できなければ即座に全 Agent をシャットダウンし中断する
