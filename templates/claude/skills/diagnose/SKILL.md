...
- **full プリセット専用**（CTO/CPO/CISO/COO エージェントがフィルタリングに必要）
- **3者承認ゲート**: CISO + CPO + COO の3者フィルタで自律実行不可領域（認証 / 暗号 / 課金構造 / ガードレール / MVV）を自動除外する（rules/autonomous-restrictions.md 準拠）
...
### 6b. COO フィルタリング（自律実行可否チェック）

COO エージェントに残った候補を渡し、`rules/autonomous-restrictions.md` の不可領域に該当するものを除外させる。
...
COO が「除外」と判定した候補はリストから除外する。

**3者承認ゲート**: ここまでで CISO + CPO + COO の3者フィルタを通過した候補のみが `/autopilot` → `/ship-parallel` での自律実行対象となる。
...
- CISO 除外: {n} 件（自己制約緩和）
- CPO 除外: {n} 件（MVV 不整合）
- COO 除外: {n} 件（不可領域 — 認証 / 暗号 / 課金構造 / ガードレール / MVV）
- 上限除外: {n} 件
- 最終候補: {n} 件
