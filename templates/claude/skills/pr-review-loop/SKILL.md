---
name: pr-review-loop
description: "PRの未解決コメントを修正してpushする。1回の実行で現在の指摘を処理して終了する。定期実行は /loop 5m /pr-review-loop で行う。「/pr-review-loop」「レビュー対応して」「PRレビュー修正して」と言った時に使用。"
---
...
```bash
/loop 5m /pr-review-loop
```

5分間隔で自動実行し、PRがマージされるまで指摘対応を繰り返す。
ユーザーは最初に1回指示するだけで、あとは放置できる。
...
/pr-review-loop --worktree <path>  # worktree 内で実行
```
