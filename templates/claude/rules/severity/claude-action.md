...
| ⚪ | Info | 情報提供のみ、対応不要（CodeRabbit デフォルト解釈） |

**注意**: 本テーブルは severity の **定義**（重大度の意味）を記録するもの。修正対象の **判定** は `.claude/rules/review-handling.md` が行う（vibecorp 運用ルール）。特に Info については、vibecorp は「重視軸該当なら対応」に拡張している（`review-handling.md` 参照）。
