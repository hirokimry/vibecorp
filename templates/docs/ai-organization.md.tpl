...
| チーム | C-Level | 分析員 | 分析員の役割 |
|--------|---------|--------|-------------|
| 法務チーム | CLO | 法務分析員（legal-analyst） | ライセンス分析・コンプライアンスチェック |
| 経理チーム | CFO | 経理分析員（accounting-analyst） | API コスト計算・予算消化率の算出 |
| セキュリティチーム | CISO | セキュリティ分析員（security-analyst） | 脆弱性スキャン・セキュリティポリシー検証 |

分析員は C-Level エージェントから呼び出され、結果を報告する。分析員が直接 docs/ を編集することはない。
...
- `.claude/knowledge/` 配下は全ロールが編集可能（ナレッジ蓄積のため）
...
|-------|------|----------------|---------------|
| Phase 0 | 準備 | なし（手動運用） | — |
| Phase 1 | 基盤導入 | CTO | minimal |
| Phase 2 | レビュー体制 | CTO + CPO | standard |
| Phase 3 | コスト・法務 | + CFO + CLO + 分析員 | full |
| Phase 4 | セキュリティ | + CISO + 分析員 | full |
| Phase 5 | フル組織 | + COO（全エージェント稼働） | full |
