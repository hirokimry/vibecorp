...
| ③ auto-resolve（自身のコメント dismiss） | ◎ ネイティブ（`request_changes_workflow` の派生として、修正済みと判定したコメントを push 時に自動 resolve） | ◎ vibecorp が `gh api` で claude-action 自身のコメントを dismiss（#466 参照） |
...
| `reviews.request_changes_workflow` | `true` | 4 契約 ② approve 切替（4 契約 ③ auto-resolve も派生として担う）|
