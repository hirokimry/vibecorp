...
> 本ドキュメントは {{PROJECT_NAME}} に導入された vibecorp プラグインの設計思想を引き継いでいます。
> {{PROJECT_NAME}} 固有のカスタマイズはこのファイルへ追記してください。
...
- **minimal**: /vibecorp:review, /vibecorp:review-loop, /vibecorp:pr-fix-loop, /vibecorp:pr, /vibecorp:commit, /vibecorp:issue, /vibecorp:ship, /vibecorp:plan, /vibecorp:branch, /vibecorp:plan-review-loop
...
- OK: minimal の `/vibecorp:pr-fix-loop` が minimal の `/vibecorp:review` を呼ぶ
...
- `auto_resolve: true` — push 時に修正済みのレビューコメントを自動 resolve する。`/vibecorp:pr-fix-loop` の「修正した指摘は返信不要」方針の前提
- `language: ja-JP` — レビューコメントを日本語で出力（`vibecorp.yml` の `language` と連動）
- 各設定値が `/vibecorp:pr-fix-loop` のどのステップに対応しているかの詳細は `docs/coderabbit-dependency.md` の「設定値と仕様要件の対応」セクションを参照
