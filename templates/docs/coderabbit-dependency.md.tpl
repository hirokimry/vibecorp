...
| スキル | 依存度 | 未導入時の挙動 |
|--------|--------|----------------|
| `/vibecorp:pr-fix-loop` | **高** | CodeRabbit レビュー待ちはスキップ。CI 失敗検知・修正は CodeRabbit 非依存で動作する |
...
| `/vibecorp:ship` | 間接 | ステップ9（`/vibecorp:pr-fix-loop`）がスキップ |
| `/vibecorp:pr` | なし | 影響なし |
...
## .coderabbit.yaml 設定値と /vibecorp:pr-fix-loop の仕様要件の対応

`.coderabbit.yaml` の各設定値は `/vibecorp:pr-fix-loop` の特定のステップが正しく動作するための前提条件である。以下に設定値・依存するステップ・理由・変更時の影響を示す。
...
| `reviews.auto_resolve.enabled` | `true` | 2.6（修正実行）, 2.8（push） | push 時に修正済みコメントを CodeRabbit が自動 resolve する。`/vibecorp:pr-fix-loop` の「修正した指摘は返信不要」方針の前提であり、手動 resolve の手間を排除する | `false` にすると修正済みの指摘が未解決のまま残り、ループが終了条件（未解決0件）を満たせなくなる |
...
| `language` | `ja-JP` | 2.4（妥当性検証） | レビュー指摘が日本語で出力されることで、`.claude/rules/review-handling.md`（捌き基準）と `severity/coderabbit.md`（severity 公式定義）に基づく妥当性判定を日本語で一貫して実行できる。`vibecorp.yml` の `language` と連動する | 言語を変更すると指摘の言語と rules の言語が不一致になり、妥当性判定の精度が低下する可能性がある |

### 設定値間の依存関係

上記の設定値は独立ではなく、組み合わせで `/vibecorp:pr-fix-loop` のワークフローを成立させている。
...
### `/vibecorp:pr-fix-loop`（依存度: 高）

`/vibecorp:pr-fix-loop` は `/loop` や `ScheduleWakeup` のような非同期スケジューラに依存せず、teammate のターン内で `gh pr view` をポーリングする同期ループとして実装されている（[根拠](#同期ループ仕様)）。
...
teammate（`/vibecorp:ship-parallel` 配下の Agent）はメッセージ駆動で idle 化すると `ScheduleWakeup` の wakeup が届かないため、`/loop 5m /vibecorp:pr-fix` 方式では PR が放置される構造欠陥があった。これを解消するため、本スキルは以下の同期ループ仕様で実装されている。
...
- `CHANGES_REQUESTED` 検知時は `/vibecorp:pr-fix` を Skill ツールで同期呼び出し
...
`/vibecorp:pr-fix-loop` と `/vibecorp:ship` のステップ9は実質スキップされるため、PR作成後は人間のワークフローに委ねる形になる。
...
- **`false` 設定時の効果**:
  ...
  - `/vibecorp:pr-fix-loop` が CodeRabbit レビュー待ちを即座にスキップ（5分待ち解消）
